...
def build_review_priority(
    ...
    reason = (
        " ".join(signal["reason"] for signal in sorted(signals, key=lambda item: (-int(item["weight"]), item["code"]))[:3])
        if signals
        else "Low-risk advisory profile based on the current review signals."
    )
