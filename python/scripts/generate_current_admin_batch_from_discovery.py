...
def candidate_snapshot(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "confidence_penalties": item.get("confidence_penalties") or [],
        "matched_keywords": item.get("matched_keywords") or [],
        "source_category": item.get("source_category"),
        "legal_status": item.get("legal_status"),
        "court_or_agency": item.get("court_or_agency"),
        "docket_number": item.get("docket_number"),
        "target_agency": item.get("target_agency") or (item.get("suggested_changes") or {}).get("target_agency"),
        "target_program": item.get("target_program") or (item.get("suggested_changes") or {}).get("target_program"),
        "implementation_stage": item.get("implementation_stage") or (item.get("suggested_changes") or {}).get("implementation_stage"),
        "mechanism_of_effect": item.get("mechanism_of_effect") or (item.get("suggested_changes") or {}).get("mechanism_of_effect"),
        "funding_signal": item.get("funding_signal") or (item.get("suggested_changes") or {}).get("funding_signal"),
        "affected_institutions": item.get("affected_institutions") or (item.get("suggested_changes") or {}).get("affected_institutions"),
