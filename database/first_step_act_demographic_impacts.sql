...
-- Candidate policy:
--   First Step Act
-- This first-pass evidence layer stays narrowly focused on the Act's
-- sentencing provisions where the strongest Black-specific outcome data is
-- publicly available from the U.S. Sentencing Commission.

SET @fsa_policy_title := 'First Step Act';
SET @fsa_policy_year := 2018;
...
SET @fsa_policy_match_count := (
  SELECT COUNT(*)
  FROM policies
  WHERE title = @fsa_policy_title
    AND year_enacted = @fsa_policy_year
);
...
SELECT
  ...
  'Congress.gov',
  NULL,
  'Legislative record for the First Step Act policy anchor.'
WHERE @fsa_policy_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
    FROM sources s
    WHERE s.policy_id = @fsa_policy_id
...
SELECT
  ...
  '2020-08-31',
  'U.S. Sentencing Commission report covering the first full year of implementation of the First Step Act, including race-specific data on Section 404 resentencing recipients.'
WHERE @fsa_policy_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
    FROM sources s
...
SELECT
  ...
  'U.S. Sentencing Commission',
  '2022-08-18',
  'U.S. Sentencing Commission retroactivity report with cumulative Section 404 resentencing data through August 18, 2022, including race-specific recipient counts.'
WHERE @fsa_policy_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
    FROM sources s
...
DROP TEMPORARY TABLE IF EXISTS tmp_first_step_act_impacts;
CREATE TEMPORARY TABLE tmp_first_step_act_impacts (
...
INSERT INTO tmp_first_step_act_impacts (
  demographic_group,
  metric_name,
  before_value,
  after_value,
...
(
  ...
  2022,
  0.8600,
  'Direct measured outcome row. The U.S. Sentencing Commission reported in August 2022 that 3,877 of the 4,212 Section 404 recipients with race data were Black, meaning Black offenders accounted for 92.0% of recorded First Step Act resentencing recipients in this dataset. This is a documented outcome count, not a full estimate of everyone potentially eligible for relief.'
),
(
...
INSERT INTO entity_demographic_impacts (
  ...
  t.confidence_score,
  t.methodology_note
FROM tmp_first_step_act_impacts t
WHERE @fsa_policy_id IS NOT NULL
...
DROP TEMPORARY TABLE IF EXISTS tmp_first_step_act_impact_sources;
CREATE TEMPORARY TABLE tmp_first_step_act_impact_sources (
  demographic_group VARCHAR(150) NOT NULL,
  metric_name VARCHAR(255) NOT NULL,
  geography VARCHAR(150) NOT NULL,
  year_before SMALLINT UNSIGNED NOT NULL,
...
INSERT INTO tmp_first_step_act_impact_sources (
  demographic_group,
  metric_name,
  geography,
  year_before,
...
(
  ...
  @fsa_retro_source_url,
  'primary',
  'USSC August 2022 retroactivity report states that 3,877 of 4,212 Section 404 recipients with race data were Black, or 92.0% of the observed recipient pool.'
),
(
  'Black federal offenders',
  'Black offenders receiving Section 404 sentence reductions under the First Step Act',
  ...
  @fsa_congress_source_url,
  'context',
  'Official Congress.gov legislative record for the First Step Act provisions that made Fair Sentencing Act relief retroactive through Section 404.'
),
(
  'Black federal crack-cocaine offenders',
  'Supporting evidence - Black share of Section 404 recipients compared with FY2018 federal crack-offense caseload',
  ...
  @fsa_year_one_source_url,
  'supporting',
  'USSC one-year implementation report found Black offenders were 91.4% of Section 404 recipients and 80.0% of crack offenders in fiscal year 2018.'
),
(
  'Black federal crack-cocaine offenders',
  'Supporting evidence - Black share of Section 404 recipients compared with FY2018 federal crack-offense caseload',
  ...
  @fsa_congress_source_url,
  'context',
  'Congress.gov record for the First Step Act legislative changes that authorized Section 404 resentencing.'
),
(
  'Section 404 sentence-reduction recipients',
  'Supporting evidence - Average sentence length after First Step Act Section 404 reductions',
...
(
  ...
  @fsa_congress_source_url,
  'context',
  'Congress.gov legislative record for the First Step Act policy anchor.'
);
...
INSERT INTO entity_demographic_impact_sources (
  ...
  t.source_role,
  t.citation_note
FROM tmp_first_step_act_impact_sources t
JOIN entity_demographic_impacts edi
  ON edi.entity_type = 'policy'
 AND edi.entity_id = @fsa_policy_id
