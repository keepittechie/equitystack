import { buildPublicBillsDataset } from "../public-bills.js";
import { getFutureBills } from "../shareable-cards.js";
...
export function buildPresidentBillValidationSample(presidents = []) {
  ...
  return [
    ...
    .map(({ bucket, item }) => ({
      ...
      current_headline_score: round(
        item.overall_ranking_score ?? item.ranking_score_total ?? item.score ?? item.normalized_score_total ?? 0
      ),
