...
export function getBlackImpactScoreMethodology() {
  return {
    ...
    summary:
      "This is the canonical Black Impact Score model. It scores documented Promise Tracker outcomes, rolls them up to promises, and aggregates them to presidents with a conservative bill-informed modifier when supported by current bill-to-promise-to-president lineage.",
    ...
    aggregation: {
      ...
      president_rollup:
        "President totals start from included promise totals across that president's scored records, then apply a bounded bill-informed modifier only when supported by current bill-to-promise-to-president lineage.",
      normalization:
        "Outcome-based president totals are scaled into a temporary -100 to 100 range using available scored outcome count, then blended modestly with validated bill-informed inputs when present.",
    },
    notes: [
      ...
      "Bill-informed inputs are capped so thin or weak legislative linkage cannot dominate the outcome-based record.",
