...
const DIRECTION_BASE_SCORES = {
  Positive: 1,
  Negative: -1,
  // Mixed and Blocked remain visible in the dataset and directional counts,
  // but stay neutral in the current numeric model until we support
  // decomposition/blocked-effect scoring without forcing speculative weights.
  Mixed: 0,
  Blocked: 0,
};
...
export function computeOutcomeScore(outcomeContext = {}) {
  ...
  const policyIntentCategory = normalizeText(
    outcomeContext.policy_intent_category ?? outcome.policy_intent_category
  ) || "unclear";
  const intentModifier = Number(outcomeContext.intent_modifier ?? outcome.intent_modifier ?? 1);
  ...
  const systemicImpactCategory = resolveSystemicImpactCategory(
    outcomeContext.systemic_impact_category ?? outcome.systemic_impact_category
  );
  const systemicImpactSummary = normalizeText(
    outcomeContext.systemic_impact_summary ?? outcome.systemic_impact_summary
  );
  const systemicPolicyTitle = normalizeText(
    outcomeContext.systemic_policy_title ?? outcome.systemic_policy_title
  );
  const systemicMultiplier = systemicMultiplierFor(systemicImpactCategory);
  ...
  return {
    outcome: {
      ...
      black_community_impact_note:
        blackCommunityImpactNote ?? outcome.black_community_impact_note ?? null,
      source_count: sourceCount,
      policy_intent_category: policyIntentCategory,
      intent_modifier: intentModifier,
      systemic_impact_category: systemicImpactCategory,
      systemic_impact_summary: systemicImpactSummary,
      systemic_policy_title: systemicPolicyTitle,
      systemic_multiplier: systemicMultiplier,
    },
