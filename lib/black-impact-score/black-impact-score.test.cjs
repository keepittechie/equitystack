...
const {
  computePresidentHeadlineScore,
  sortPresidentsByHeadlineScore,
} = jiti("./presidentHeadlineScore.js");
const { buildPresidentRankExplanation } = jiti("./presidentRankExplanation.js");
const {
...
test("president rollup aggregates promise totals and normalizes by scored outcomes", () => {
  ...
  assert.equal(presidents[0].systemic_raw_score, null);
  ...
  assert.equal(presidents[0].overall_ranking_score, 1.55);
  assert.equal(presidents[0].coverage_adjusted_score, 3.88);
  assert.equal(presidents[0].raw_average_score, 10);
  assert.equal(presidents[0].evidence_confidence, "VERY LOW");
  assert.equal(presidents[0].score_confidence, "VERY LOW");
  assert.equal(presidents[0].score_confidence_basis, 2);
  assert.equal(presidents[0].display_score_total, 3.88);
...
test("coverage scaling dampens low-sample display scores without changing raw normalized scores", () => {
  ...
  const twoOutcomes = applyCoverageDisplayScore(100, 2);
  const tenOutcomes = applyCoverageDisplayScore(100, 10);
  ...
  assert.equal(coverageConfidenceFactor(0), 0);
  assert.equal(coverageConfidenceFactor(1), 0.2447);
  assert.equal(coverageConfidenceFactor(2), 0.3878);
  assert.equal(coverageConfidenceFactor(10), 0.8464);
  assert.equal(oneOutcome.display_score, 24.47);
  assert.equal(twoOutcomes.display_score, 38.78);
  assert.equal(tenOutcomes.display_score, 84.64);
...
test("public president headline score applies an extra sparse-record penalty for low confidence leaders", () => {
  const lowCoverage = computePresidentHeadlineScore({
    normalizedScoreTotal: 68,
    outcomeCount: 5,
    scoreConfidence: "LOW",
  });
  const highCoverage = computePresidentHeadlineScore({
    normalizedScoreTotal: 31.79,
    outcomeCount: 24,
    scoreConfidence: "HIGH",
  });

  assert.equal(lowCoverage.coverage_adjusted_score, 43);
  assert.equal(lowCoverage.headline_score, 24.94);
  assert.equal(lowCoverage.sparse_penalty_applied, true);
  assert.equal(highCoverage.headline_score, 31.79);
  assert.equal(highCoverage.sparse_penalty_applied, false);
});

test("headline score handles zero, tiny, medium, and large outcome counts safely", () => {
  const zero = computePresidentHeadlineScore({
    normalizedScoreTotal: 100,
    outcomeCount: 0,
  });
  const tiny = computePresidentHeadlineScore({
    normalizedScoreTotal: 100,
    outcomeCount: 2,
    scoreConfidence: "VERY LOW",
  });
  const medium = computePresidentHeadlineScore({
    normalizedScoreTotal: 40,
    outcomeCount: 10,
    scoreConfidence: "MEDIUM",
  });
  const large = computePresidentHeadlineScore({
    normalizedScoreTotal: 24,
    outcomeCount: 25,
    scoreConfidence: "HIGH",
  });

  assert.equal(zero.headline_score, 0);
  assert.equal(zero.coverage_adjusted_score, 0);
  assert.equal(zero.score_confidence, "VERY LOW");
  assert.equal(tiny.headline_score, 15.51);
  assert.equal(medium.coverage_adjusted_score, 33.86);
  assert.equal(medium.headline_score, 33.86);
  assert.equal(large.headline_score, 24);
});

test("public president ranking favors fuller high-confidence records over sparse low-confidence records", () => {
  const ranked = sortPresidentsByHeadlineScore([
    {
      president: "George H. W. Bush",
      normalized_score_total: 68,
      outcome_count: 5,
      score_confidence: "LOW",
    },
    {
      president: "Gerald Ford",
      normalized_score_total: 65.45,
      outcome_count: 3,
      score_confidence: "LOW",
    },
    {
      president: "Barack Obama",
      normalized_score_total: 48.15,
      outcome_count: 16,
      score_confidence: "HIGH",
    },
    {
      president: "Joseph R. Biden Jr.",
      normalized_score_total: 31.79,
      outcome_count: 24,
      score_confidence: "HIGH",
    },
  ]);
  ...
  assert.deepEqual(
    ranked.map((item) => item.president),
    ["Barack Obama", "Joseph R. Biden Jr.", "George H. W. Bush", "Gerald Ford"]
  );
});

test("president rank explanation exposes raw average, outcome mix, confidence, and penalty state", () => {
  const lowCoverage = buildPresidentRankExplanation({
    raw_average_score: 68,
    coverage_adjusted_score: 43,
    overall_ranking_score: 24.94,
    outcome_count: 5,
    score_confidence: "LOW",
    direction_breakdown: {
      Positive: 4,
  ...
  const highCoverage = buildPresidentRankExplanation({
    raw_average_score: 48.15,
    coverage_adjusted_score: 48.15,
    overall_ranking_score: 48.15,
    outcome_count: 16,
    score_confidence: "HIGH",
    direction_breakdown: {
      Positive: 9,
  ...
  assert.equal(lowCoverage.coverage_penalty_applied, true);
  assert.equal(lowCoverage.direction_mix_label, "Positive 4 • Negative 0 • Mixed 0 • Blocked 1");
  assert.equal(lowCoverage.items[0].value, "5 outcomes");
  assert.equal(lowCoverage.items[2].value, "LOW");
  assert.equal(lowCoverage.items[4].value, "68.00");
  assert.equal(lowCoverage.items[5].value, "24.94");
  assert.match(lowCoverage.summary, /raw average score is 68\.00/i);
  assert.equal(highCoverage.coverage_penalty_applied, false);
  assert.match(highCoverage.note, /lower raw average can still rank higher/i);
});
