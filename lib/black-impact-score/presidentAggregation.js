...
} from "./presidentHeadlineScore.js";
...
export function aggregatePresidentFromOutcomes(records = [], options = {}) {
  ...
  return [...byPresident.values()]
    .map((summary) => {
      ...
      const headlineScore = computePresidentHeadlineScore({
        normalizedScoreTotal: blendedScore.normalized_score_total,
        outcomeCount: summary.outcome_count,
        scoreConfidence: displayScore.score_confidence,
      });
      ...
      const presidentSummary = {
        ...
        ranking_score_total: headlineScore.headline_score,
        overall_ranking_score: headlineScore.headline_score,
        ranking_score_method: headlineScore.ranking_method,
        ranking_sparse_confidence_weight: headlineScore.sparse_confidence_weight,
        ranking_sparse_penalty_applied: headlineScore.sparse_penalty_applied,
        ...
        display_score: headlineScore.coverage_adjusted_score,
        display_score_total: headlineScore.coverage_adjusted_score,
        coverage_adjusted_score: headlineScore.coverage_adjusted_score,
        raw_average_score: blendedScore.normalized_score_total,
        score_confidence: headlineScore.score_confidence,
        evidence_confidence: headlineScore.score_confidence,
        score_confidence_factor: headlineScore.score_confidence_factor,
        score_confidence_basis: headlineScore.score_confidence_basis,
        low_coverage_warning: headlineScore.low_coverage_warning,
        ...
        explanation_summary: [
          ...
          blendedScore.bill_blend_weight > 0
            ? `${blendedScore.bill_influence_label}; bill-linked inputs adjust the raw average score by ${blendedScore.blended_score_modifier >= 0 ? "+" : ""}${blendedScore.blended_score_modifier}.`
            : null,
        ]
          .filter(Boolean)
          .join(" "),
    ...
    .sort((a, b) => {
      const rankingDiff = getPresidentHeadlineScoreValue(b) - getPresidentHeadlineScoreValue(a);
      if (rankingDiff !== 0) {
        return rankingDiff;
      }
