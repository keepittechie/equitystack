...
function titleForTool(toolName) {
  const titles = {
    ...
    run_current_admin: "MCP Current-Admin Run",
...
export const MCP_TOOLS = [
  ...
  {
    name: "run_current_admin",
    description: "Run the canonical current-admin pipeline through the CLI runner.",
    inputSchema: {
      type: "object",
      properties: {
...
export async function callMcpTool(name, args = {}) {
  ...
  if (name === "run_current_admin") {
    ...
    return runLoggedCliAction({
      ...
      input,
      args,
      summary: "Current-admin run requested by MCP.",
      successSummary: "Current-admin run completed.",
      timeoutMs: 60 * 60 * 1000,
    });
  }
