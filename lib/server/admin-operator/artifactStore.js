...
async function deleteArtifactRecordsFile(workflowFamily) {
  const records = await listArtifactRecordsFile({ workflowFamily });
  await Promise.all(
    records.map(async (record) => {
      await fs.rm(buildArtifactRecordPath(record.id), { force: true }).catch(() => {});
    })
  );
}

export async function deleteArtifactRecordsByWorkflowFamily(workflowFamily) {
  ...
  await withOperatorDbFallback(
    async (db) => {
      await db.execute(`DELETE FROM operator_artifacts WHERE workflow_family = ?`, [normalizedFamily]);
    },
    async () => deleteArtifactRecordsFile(normalizedFamily)
  );

  await deleteArtifactRecordsFile(normalizedFamily);
}
