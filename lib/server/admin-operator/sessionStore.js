...
async function deleteWorkflowSessionRecordsFile(workflowFamily) {
  const records = await listWorkflowSessionRecordsFile({ workflowFamily, activeOnly: false });
  await Promise.all(
    records.map(async (record) => {
      await fs.rm(buildWorkflowSessionPath(record.id), { force: true }).catch(() => {});
    })
  );
}

export async function deleteWorkflowSessionRecordsByWorkflowFamily(workflowFamily) {
  ...
  await withOperatorDbFallback(
    async (db) => {
      await db.execute(`DELETE FROM operator_workflow_sessions WHERE workflow_family = ?`, [normalizedFamily]);
    },
    async () => deleteWorkflowSessionRecordsFile(normalizedFamily)
  );

  await deleteWorkflowSessionRecordsFile(normalizedFamily);
}
