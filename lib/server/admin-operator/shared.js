...
export function getReservedExecutorMetadata() {
  return {
    executor_model: normalizeString(process.env.EQUITYSTACK_EXECUTOR_MODEL) || "rnj-1:latest",
    executor_backend: normalizeString(process.env.EQUITYSTACK_EXECUTOR_BACKEND) || "ollama",
...
export function getRemoteExecutorTransportConfig() {
  ...
  const ollamaUrl =
    ...
    `http://${host}:11434`;
  return {
    transport: normalizeString(process.env.EQUITYSTACK_REMOTE_EXECUTOR_TRANSPORT) || "ollama_http",
    target: ollamaUrl,
    host,
    ollamaUrl,
  };
...
export function buildExecutionRuntimeMetadata(executionMode, overrides = {}) {
  ...
  if (mode === EXECUTION_MODES.REMOTE_EXECUTOR) {
    ...
    return {
      ...
      executor_transport_url: remoteTransport.ollamaUrl,
      ...overrides,
    };
  }
