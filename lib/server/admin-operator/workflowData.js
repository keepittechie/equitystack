...
function getLegislativeBlockedGuidance(
  ...
  if (pendingBundleApprovals > 0 || approvedBundleActions > 0) {
    return {
      reason:
        pendingBundleApprovals > 0
          ...
          : `${approvedBundleActions} approved legislative bundle action(s) are ready for apply preview.`,
      href: bundleHref,
...
export function buildLegislativeWorkflowTracker(
  ...
  const actionableBundleActions = Math.max(
    toSafeNumber(counts.actionable_bundle_actions, 0),
    pendingBundleApprovals + approvedPendingActions
  );
  ...
  if (aiReviewComplete && manualReviewCount === 0) {
    if (actionableBundleActions > 0) {
      bundleStatus = "current";
      bundleReason =
        pendingBundleApprovals > 0
          ? `${pendingBundleApprovals} legislative bundle action(s) still need an explicit approve or dismiss decision.`
          : `${approvedPendingActions} approved legislative bundle action(s) are ready for apply preview.`;
  ...
  if (aiReviewComplete && manualReviewCount === 0 && actionableBundleActions === 0) {
