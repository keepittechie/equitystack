  ...
  if (state === "REVIEW_READY") {
    if (outcomeSummary?.workflow_status === "completed_with_fallback") {
      return `AI review failed and fallback drove ${toSafeNumber(
        ...
        0
      )}/${toSafeNumber(outcomeSummary?.ai_status?.total_items, 0)} legislative item(s). ${manualReviewCount} item(s) require manual review.`;
    }
    if (toSafeNumber(outcomeSummary?.ai_status?.fallback_used, 0) > 0) {
      return `Legislative review used fallback for ${toSafeNumber(
        outcomeSummary?.ai_status?.fallback_used,
        0
      )}/${toSafeNumber(outcomeSummary?.ai_status?.total_items, 0)} item(s). ${manualReviewCount} item(s) require human review.`;
    }
    if (manualReviewCount > 0) {
      return `${manualReviewCount} legislative item(s) require manual review before the workflow can continue.`;
    }
...
function getCurrentAdminBlockedGuidance(workspace, { reviewHref, blockerHref }) {
  ...
  if (reviewPageFix) {
    return {
      ...
      label: importReadiness?.needs_queue_sync ? "Finalize decisions" : "Open current-admin review",
    };
  }
...
export function buildCurrentAdminWorkflowTracker(
    ...
    if (decisionLogExists && queueExists) {
      ...
      if (actionPermissions.finalize?.allowed === false) {
        ...
        finalizeReason =
          ...
          "Finalize is blocked until the review workspace is complete.";
        ...
        finalizeAction = buildTrackerLinkConfig(`${reviewHref}#review-actions`, "Finalize decisions");
      }
    }
  }
  steps.push(
  ...
  if (decisionLogExists && queueExists) {
    if (
      ...
      readinessReason = "The decision log and manual review queue are ready for the canonical pre-commit check.";
      readinessAction =
...
function getLegislativeBlockedGuidance(
  ...
  const blockers = toArray(workspace?.blockers).filter(Boolean);
  const artifactStatus = workspace?.artifact_status || {};
  const missingArtifact = Object.values(artifactStatus).find(
    (artifact) =>
      artifact &&
      !artifact.exists &&
      ["review_bundle", "manual_review_queue", "apply_report", "import_report"].includes(
        normalizeString(artifact.key)
      )
  );
  const aiFailed =
  ...
  if (manualReviewCount > 0 || aiFailed) {
    return {
      reason:
        ...
        `${manualReviewCount} legislative item(s) still require manual review before the workflow can continue.`,
      href: reviewHref,
  ...
  if (pendingBundleApprovals > 0) {
    return {
      reason:
        `${pendingBundleApprovals} legislative bundle action(s) still need an explicit approve or dismiss decision.`,
      href: bundleHref,
  ...
  return {
    reason: "The legislative workflow is waiting for the next guarded operator checkpoint.",
    href: blockerHref,
    label: "Inspect legislative blocker",
  };
}
...
export function buildLegislativeWorkflowTracker(
  ...
  if (aiReviewComplete) {
    if (manualReviewCount > 0) {
      ...
      manualReviewReason = fallbackOnly
        ? normalizeString(outcomeSummary.user_message) ||
          `${manualReviewCount} legislative item(s) require manual review because AI fell back.`
        : `${manualReviewCount} legislative item(s) still require manual review.`;
      manualReviewAction = buildTrackerLinkConfig(reviewHref, "Open legislative review");
    } else {
      manualReviewStatus = "complete";
      manualReviewReason = "No legislative manual-review items remain in the canonical queue.";
    }
  }
  steps.push(
    buildCurrentAdminTrackerStep({
  ...
  if (aiReviewComplete && manualReviewCount === 0) {
    if (pendingBundleApprovals > 0) {
      ...
      bundleReason = `${pendingBundleApprovals} bundle action(s) still need an explicit approve or dismiss decision.`;
      bundleAction = buildTrackerLinkConfig(bundleHref, "Open bundle approval");
    } else {
      bundleStatus = "complete";
      bundleReason = "No pending bundle-approval decisions remain.";
    }
  }
  steps.push(
    buildCurrentAdminTrackerStep({
...
function getLegislativeNextActionReason(state, workspace = null) {
  ...
  if (state === "REVIEW_READY") {
    ...
    if (manualReviewCount > 0 && pendingBundleApprovals === 0) {
      return "The manual-review queue still contains items that need human review even though no bundle approvals are pending yet.";
    }
...
function buildLegislativeReviewQueueSnapshots(workspace, sessionRecord, artifactsByKey) {
  ...
  const bundleApprovalItems = toArray(workspace?.operator_actions)
...
function suppressionMatchesSession(suppression, sessionSnapshot) {
  ...
  return (
    normalizeString(suppression.workflow_family) === sessionSnapshot.workflowFamily &&
    normalizeString(suppression.canonical_session_key) === sessionSnapshot.canonicalSessionKey
  );
}
...
export async function getCommandCenterSummary() {
  ...
  const legislativeWorkflowTracker =
    legislativeSession?.workflowTracker ||
    buildLegislativeWorkflowTracker(legislativeRefresh?.workspace || null, {
      sessionId: legislativeSession?.id || legislativeRefresh?.session?.id || "legislative:review-bundle",
    });
