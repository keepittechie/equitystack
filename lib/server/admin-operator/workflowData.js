...
function getLegislativeActivityTimestamp(workspace) {
  return (
    workspace?.import_report?.generated_at ||
    workspace?.repair_report?.generated_at ||
    workspace?.apply_report?.generated_at ||
    workspace?.approved_seed_file?.generated_at ||
    workspace?.review_bundle?.generated_at ||
    workspace?.manual_review_queue?.generated_at ||
    workspace?.ai_review?.generated_at ||
    workspace?.candidate_discovery?.generated_at ||
    workspace?.partial_suggestions?.generated_at ||
    workspace?.pipeline_report?.generated_at ||
    workspace?.pipeline_report?.started_at ||
    null
  );
}

function isStaleLegislativePipelineOnlyWorkspace(workspace) {
  const seedRowCount = toSafeNumber(workspace?.approved_seed_file?.row_count, 0);
  ...
  const approvedPendingActions = toSafeNumber(workspace?.counts?.approved_pending_actions, 0);
  const hasOnlyPipelineArtifact =
    Boolean(workspace?.pipeline_report) &&
    !workspace?.review_bundle &&
    !workspace?.ai_review &&
    !workspace?.manual_review_queue &&
    !workspace?.partial_suggestions &&
    !workspace?.candidate_discovery &&
    !workspace?.apply_report &&
    !workspace?.repair_report &&
    !workspace?.import_report &&
    seedRowCount === 0;

  return (
    hasOnlyPipelineArtifact &&
    actionableActions === 0 &&
    pendingReview === 0 &&
    approvedPendingActions === 0 &&
    isOlderThanHours(getLegislativeActivityTimestamp(workspace), 24)
  );
}

function shouldTreatLegislativeSessionAsActive(
  workspace,
  ...
  const relatedJobStatus = normalizeString(relatedJob?.status);
  if (["queued", "running", "failed", "blocked"].includes(relatedJobStatus)) {
    return true;
  }

  const manualReviewCount = toSafeNumber(workspace?.counts?.manual_review_items, 0);
  const pendingBundleApprovals = toSafeNumber(workspace?.counts?.pending_unreviewed_actions, 0);
  const approvedPendingActions = toSafeNumber(workspace?.counts?.approved_pending_actions, 0);
  if (manualReviewCount > 0 || pendingBundleApprovals > 0 || approvedPendingActions > 0) {
    return true;
  }

  const permissions = workspace?.action_permissions || {};
  if (
    permissions.run_apply_dry_run?.allowed ||
    permissions.apply_bundle?.allowed ||
    permissions.run_import_dry_run?.allowed ||
    permissions.apply_import?.allowed
  ) {
    return true;
  }

  const state = normalizeString(workspace?.workflow_status) || "DISCOVERY_READY";
  const freshActivity = !isOlderThanHours(getLegislativeActivityTimestamp(workspace), 24);
  if (state === "BLOCKED") {
    return freshActivity;
  }
  if (state === "DISCOVERY_READY") {
    return Boolean(workspace?.pipeline_report) && freshActivity;
  }
...
function buildLegislativeSessionSnapshot(
  ...
  return {
    ...
    canonicalState: state,
    active: shouldTreatLegislativeSessionAsActive(workspace, { relatedJobRunId, relatedJob }),
    recommendedActionId,
...
function shouldSuppressIdleBaselineState({
  ...
  if (workflowFamily === "legislative") {
    ...
    if (isStaleLegislativePipelineOnlyWorkspace(workspace)) {
      return true;
    }
...
export async function getCommandCenterSummary() {
  ...
  const activeSessionIds = new Set(enrichedSessions.map((session) => session.id).filter(Boolean));
  ...
  const missingArtifacts = enrichedArtifacts.filter(
    (artifact) =>
      activeSessionIds.has(artifact.sessionId) &&
