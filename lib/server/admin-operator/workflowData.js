...
import { listArtifactRecords, upsertArtifactRecord } from "./artifactStore.js";
import {
...
function buildCurrentAdminSessionSnapshot(
  ...
  return {
    ...
    href: `/admin/workflows/${encodeURIComponent(`current-admin:${batchName}`)}`,
    operatorSurfaceHref: "/admin/current-admin-review",
...
export async function refreshWorkflowFamilyState(
  ...
  const artifactSnapshots =
    workflowFamily === "current-admin"
      ? buildCurrentAdminArtifactSnapshots(workspace, sessionRecord, relatedJobRunId, relatedJob)
      : buildLegislativeArtifactSnapshots(workspace, sessionRecord, relatedJobRunId, relatedJob);

  const artifacts = await Promise.all(
    artifactSnapshots.map((record) => upsertArtifactRecord(record, { relatedJobRunId }))
  );
