...
import { getOperatorVerificationBanner } from "./verificationService.js";
import {
...
const DASHBOARD_BUCKETS = {
  ...
  awaitingHumanReview: {
    ...
    description: "Sessions and queue items waiting on explicit operator review or approval.",
  },
  readyToRun: {
    id: "readyToRun",
    title: "Ready To Run",
...
function buildSessionSnapshotCard(session, { jobs = [], artifacts = [], schedules = [] } = {}) {
  ...
  const missingArtifacts = artifacts.filter(
    (artifact) => artifact.sessionId === session.id && !artifact.exists
  );
  ...
  return {
    ...
    priority: buildPriorityMetadata(
      ...
      session.canonicalState === "BLOCKED"
        ...
        : session.reviewPendingCount > 0
          ? "This session has active operator review work waiting."
          : session.recommendedActionId
...
function summarizeCurrentAdminState(state, workspace) {
  ...
  if (state === "BLOCKED") {
    return `${workspace.batch.batch_name} is blocked by pre-commit readiness.${buildReviewRuntimeNote(workspace?.review_runtime)}`;
  }
...
function summarizeLegislativeState(state, workspace) {
  ...
  return `Legislative is ${state} with ${toSafeNumber(
    workspace?.counts?.pending_unreviewed_actions,
    0
  )} pending bundle actions.${buildReviewRuntimeNote(workspace?.review_runtime)}`;
}

function getCurrentAdminNextActionReason(state) {
  if (state === "DISCOVERY_READY") {
    return "No active batch is ready yet, so the next step is to run discovery and prepare the next batch.";
  }
  if (state === "REVIEW_READY") {
    return "The review artifact is ready and explicit operator decisions are still required.";
  }
  if (["QUEUE_READY", "PRECOMMIT_READY"].includes(state)) {
    return "Operator decisions are logged and the guarded apply path can run pre-commit and dry-run checks.";
  }
  if (state === "BLOCKED") {
    return "The last guarded apply path stopped at a blocking checkpoint and needs operator attention.";
  }
  if (state === "IMPORT_READY") {
    return "A dry-run import already exists, so only an explicitly confirmed final apply remains.";
  }
  return "The next step is derived from the canonical current-admin workflow state.";
}
...
export function buildCurrentAdminWorkflowTracker(
  ...
  const normalizedArtifact = artifactStatus.normalized_batch || null;
  ...
  const normalizedExists = Boolean(normalizedArtifact?.exists);
  ...
  const topBlocker =
    ...
    normalizeString(preCommitArtifact?.summary) ||
    "";
  ...
  const dryRunAction = buildTrackerActionConfig({
    ...
    label: "Run pre-commit / dry-run",
    input: batchInput,
    context: actionContext,
    helperText: "Runs the guarded dry-run path and preserves apply confirmation separately.",
  });
  ...
  if (hasBatch) {
    if (reviewExists) {
      ...
      runActionConfig = buildTrackerLinkConfig(sessionHref, "Inspect session");
    } else {
      runStatus = "current";
      runReason =
        state === "NORMALIZED"
          ? "Normalization is complete, but the review artifact has not been generated yet."
          : "The batch is ready, but the wrapped current-admin run has not been completed yet.";
      runActionConfig = runAction;
    }
  }
  steps.push(
    buildCurrentAdminTrackerStep({
      id: "run_current_admin",
      title: "Run current-admin",
      status: runStatus,
      reason: runReason,
    ...
    if (pendingReview > 0 || state === "REVIEW_READY") {
      ...
      reviewReason =
        pendingReview > 0
          ...
          : "The review artifact is ready and waiting for explicit operator review.";
      reviewAction = buildTrackerLinkConfig(reviewHref, "Open review");
    } else {
  ...
  let finalizeReason = "Decision logging becomes available after operator review decisions are complete.";
  let finalizeAction = null;
  if (reviewExists && pendingReview === 0) {
    if (decisionLogExists && queueExists) {
      finalizeStatus = "complete";
      finalizeReason = "The canonical decision log and manual review queue are already present.";
    } else {
      finalizeStatus = "current";
      finalizeReason =
        "All operator decisions are recorded, but the canonical decision log has not been finalized yet.";
      finalizeAction = buildTrackerLinkConfig(reviewHref, "Open review page");
    }
  }
  steps.push(
    buildCurrentAdminTrackerStep({
      id: "decision_log_finalized",
      title: "Decision Log Finalized",
  ...
  let readinessReason = "Pre-commit and dry-run readiness begin after the queue is finalized.";
  ...
  if (decisionLogExists && queueExists) {
    if (state === "BLOCKED") {
      ...
      readinessAction = buildTrackerLinkConfig(blockerHref, "Inspect blocker");
    } else if (["QUEUE_READY", "PRECOMMIT_READY"].includes(state)) {
      readinessStatus = "current";
      readinessReason =
        state === "PRECOMMIT_READY"
          ? "Pre-commit is import-ready. The next valid step is the guarded dry-run path."
          : "The decision log and manual review queue are ready for the guarded dry-run path.";
      readinessAction =
        actionPermissions.run_precommit?.allowed || actionPermissions.run_import_dry_run?.allowed
          ? dryRunAction
          : buildTrackerLinkConfig(reviewHref, "Open review page");
  ...
  steps.push(
    buildCurrentAdminTrackerStep({
      ...
      href: readinessStatus === "blocked" ? blockerHref : reviewHref,
    })
  );
  ...
  steps.push(
    buildCurrentAdminTrackerStep({
      id: "admin_approval_final_apply",
      title: "Admin Approval / Final Apply",
      status: applyStatus,
      reason: applyReason,
      action: applyAction,
      href: reviewHref,
  ...
  let validationReason = "Validation completes the workflow after apply succeeds.";
  let validationAction = null;
  if (validationExists) {
    validationStatus = "complete";
    validationReason = "The validation report is present and the workflow is complete.";
  } else if (importApplyExists || state === "COMPLETE") {
    validationStatus = "current";
    validationReason =
      "Apply is complete, but the validation report has not been refreshed yet.";
    validationAction =
      actionPermissions.validate_import?.allowed && validateAction
        ? validateAction
        : buildTrackerLinkConfig(sessionHref, "Open session");
  }
  steps.push(
    buildCurrentAdminTrackerStep({
  ...
  const nextStep = currentStep || blockedStep || steps.find((step) => step.status === "pending") || null;
  const operatorSurfaceHref =
    nextStep?.href ||
    (preferSurfaceHref
      ? blockedStep
        ? blockerHref
        : ["operator_review", "decision_log_finalized", "precommit_apply_readiness", "admin_approval_final_apply"].includes(
              nextStep?.id || ""
            )
          ? reviewHref
          : sessionHref
  ...
  return {
    ...
    summary:
      nextStep?.reason ||
      getCurrentAdminNextActionReason(state),
    steps,
  };
}

function getLegislativeNextActionReason(state) {
  if (state === "DISCOVERY_READY") {
    return "The legislative surface is waiting for the next wrapped run to rebuild the review bundle.";
  }
  if (state === "REVIEW_READY") {
    return "The review bundle is ready and still needs explicit operator review decisions.";
  }
  if (state === "APPLY_READY") {
...
function buildCurrentAdminSessionSnapshot(
  ...
  const recommendedActionId =
    state === "DISCOVERY_READY"
      ...
      : state === "REVIEW_READY"
        ? "currentAdmin.review"
        : ["QUEUE_READY", "PRECOMMIT_READY", "BLOCKED", "IMPORT_READY"].includes(state)
          ? "currentAdmin.apply"
          : "currentAdmin.status";
  ...
  return {
    ...
    metadataJson: {
      ...
      counts: {
        pendingReview: toSafeNumber(workspace?.counts?.pending, 0),
        approved: toSafeNumber(workspace?.counts?.approved, 0),
        blockedReviewItems: toSafeNumber(workspace?.counts?.blocked, 0),
      ...
      next_action_reason: getCurrentAdminNextActionReason(state),
...
function buildLegislativeSessionSnapshot(
  ...
  return {
    ...
    operatorSurfaceHref: "/admin/legislative-workflow",
    relatedJobRunIds: relatedJobRunId ? [relatedJobRunId] : [],
    metadataJson: {
      executor: runtime.executor,
      execution_mode: runtime.execution_mode,
      ...
      blockers: workspace?.blockers || [],
      counts: {
        pendingReview: toSafeNumber(workspace?.counts?.pending_unreviewed_actions, 0),
      ...
      next_action_reason: getLegislativeNextActionReason(state),
      last_refreshed_at: new Date().toISOString(),
    },
  };
}
...
function buildCurrentAdminReviewQueueSnapshots(workspace, sessionRecord, artifactsByKey) {
  ...
  const reviewItems = toArray(workspace?.review_items)
    ...
    .map((item) => ({
      ...
      detail:
        ...
        "Explicit operator review is still required.",
      href: reviewHref,
      active: true,
      metadataJson: {
        slug: item.slug || null,
  ...
  if (sessionRecord.canonicalState === "BLOCKED") {
    readinessItems.push({
      ...
      detail:
        workspace?.latest_pre_commit_review?.readiness_explanation ||
        workspace?.blockers?.[0] ||
        "Pre-commit is blocked and the apply path must stop.",
      href: blockedHref,
      active: true,
      metadataJson: {
...
function buildLegislativeReviewQueueSnapshots(workspace, sessionRecord, artifactsByKey) {
  ...
  return toArray(workspace?.operator_actions)
...
export async function listWorkflowSessions() {
  ...
  return records.map(enrichSessionRecord);
}
...
export async function getCommandCenterSummary() {
  const [currentAdminRefresh] = await refreshOperatorState();
  const [sessions, reviewQueue, allJobs, artifacts, rawSchedules] = await Promise.all([
  ...
  const currentAdminWorkflowTracker =
    ...
    buildCurrentAdminWorkflowTracker(currentAdminRefresh?.workspace || null, {
      sessionId: currentAdminSession?.id || currentAdminRefresh?.session?.id || "",
    });
  ...
  const missingArtifacts = enrichedArtifacts.filter((artifact) => !artifact.exists);
  ...
  const blockedSessionItems = sessionCards
    ...
    .map((session) =>
      buildDashboardItem({
        ...
        priorityReason:
          session.workflowFamily === "current-admin"
            ? "Blocked current-admin workflow state stops the guarded apply path and should be checked early."
