...
async function deleteSystemSignalsFile(workflowFamily) {
  const records = await listSystemSignalsFile({ activeOnly: false });
  await Promise.all(
    records
      .filter((record) => !workflowFamily || record.workflowFamily === workflowFamily)
      .map(async (record) => {
        await fs.rm(buildSystemSignalPath(record.id), { force: true }).catch(() => {});
      })
  );
}

export async function deleteSystemSignalsByWorkflowFamily(workflowFamily) {
  ...
  await withOperatorDbFallback(
    async (db) => {
      await db.execute(`DELETE FROM operator_system_signals WHERE workflow_family = ?`, [normalizedFamily]);
    },
    async () => deleteSystemSignalsFile(normalizedFamily)
  );

  await deleteSystemSignalsFile(normalizedFamily);
}
