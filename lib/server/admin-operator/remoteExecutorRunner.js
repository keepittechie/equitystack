...
function buildRemoteExecutorEnvOverrides(runtimeMetadata) {
  ...
  const llmEndpoint =
    ...
    normalizeString(transport.openaiBaseUrl) ||
    "";

  return {
    EQUITYSTACK_LLM_ENDPOINT: llmEndpoint,
    EQUITYSTACK_OPENAI_BASE_URL: llmEndpoint,
...
export async function runRemoteExecutorAction({
  ...
  const transportTarget = transport.llmEndpoint || transport.openaiBaseUrl || transport.target || "";
