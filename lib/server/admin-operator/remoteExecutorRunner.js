...
function buildRemoteExecutorEnvOverrides(runtimeMetadata) {
  ...
  const model =
    normalizeString(runtimeMetadata?.executor_model) || "rnj-1:latest";
  const ollamaUrl =
  ...
  return {
    EQUITYSTACK_OLLAMA_URL: ollamaUrl,
...
function classifyRemoteExecutorFailure(result) {
  ...
  const looksLikeRemoteBackendIssue =
    ...
    message.includes("/api/generate") ||
    message.includes("/api/tags") ||
...
export async function runRemoteExecutorAction({
  ...
  const transportEvents = [
    `Remote executor target: ${transport.ollamaUrl} (${transport.transport}).`,
    "Canonical CLI is running locally against the reserved remote Ollama backend.",
  ...
  if (runResult.ok) {
    ...
    return {
      ...
      runtimeMetadata: {
        ...
        executor_transport_target: transport.ollamaUrl,
        executor_transport_url: transport.ollamaUrl,
      },
      transportEvents,
      transportReport,
    };
