...
const REMOTE_WORKFLOW_PROBE_ACTION_ID = "currentAdmin.status";
...
function extractProviderResponseText(payload, fallbackText = "") {
  ...
  if (firstChoice && typeof firstChoice === "object") {
    if (typeof firstChoice?.message?.content === "string") {
      return normalizeString(firstChoice.message.content);
    }
    if (typeof firstChoice.text === "string") {
      return normalizeString(firstChoice.text);
    }
  }
  return normalizeString(fallbackText);
}
...
async function verifyRemoteExecutorHealthChecks() {
  ...
  const llmEndpoint =
    normalizeString(transport.llmEndpoint) || normalizeString(transport.openaiBaseUrl);
  const modelsEndpoint = normalizeString(transport.modelsEndpoint);
  ...
  checks.push(
    buildCheck({
      ...
      details: `Host: ${executor.executor_host || "-"} | Model: ${executor.executor_model || "-"} | URL: ${llmEndpoint || "-"}`,
      recommendedNextStep:
        normalizeString(executor.executor_host) &&
        normalizeString(executor.executor_model) &&
        llmEndpoint
  ...
  if (modelsEndpoint) {
    try {
      const { response, payload, text } = await fetchJsonWithTimeout(modelsEndpoint);
      if (!response.ok) {
          ...
          buildCheck({
            ...
            recommendedNextStep:
              failureCategory === "auth_config"
                ? "Verify access controls or proxy configuration for the LLM provider."
                : "Verify LLM provider service health.",
            category: "remote_executor",
      ...
      checks.push(
        buildCheck({
          ...
          name: "Remote executor backend",
          status: "passed",
          summary: "The app host can reach the configured LLM provider backend.",
          details: `Verified ${modelsEndpoint} responded successfully.`,
          recommendedNextStep: "Verify that the reserved model is available.",
          category: "remote_executor",
        })
      );
      ...
      const modelNames = models
        .map((model) => (typeof model === "string" ? model : normalizeString(model?.name || model?.id)))
        .filter(Boolean);
      ...
      checks.push(
        buildCheck({
          ...
          recommendedNextStep: modelPresent
            ...
            : `Load ${executor.executor_model} on the configured LLM provider and rerun verification.`,
          category: "remote_executor",
          metadata: {
      ...
      checks.push(
        buildCheck({
          ...
          name: "Remote executor backend",
          status: "failed",
          summary: "The app host could not reach the configured LLM provider backend.",
          details: normalizeString(error?.message) || "Unknown remote executor connectivity failure.",
          recommendedNextStep: "Verify LLM provider endpoint and network access.",
          category: "remote_executor",
    ...
    checks.push(
      buildCheck({
        ...
        name: "Reserved model availability",
        status: "warning",
        summary: "No model-list endpoint is configured for the LLM provider.",
        details: "Skipping model-list verification; the safe generation probe will verify the configured model.",
        recommendedNextStep: "Set EQUITYSTACK_LLM_MODELS_ENDPOINT if model-list verification is required.",
        category: "remote_executor",
        metadata: { failureCategory: "models_endpoint_unconfigured" },
      })
    );
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      checks.push(
        buildCheck({
          ...
          recommendedNextStep: "Inspect LLM provider health and model loading state.",
          category: "remote_executor",
          metadata: { failureCategory, statusCode: response.status },
        })
      );
    ...
    const responseText = extractProviderResponseText(payload, text);
    const passed = responseText.toUpperCase().includes("READY");
    checks.push(
      buildCheck({
        id: "remote-executor-probe",
        name: "Remote executor safe probe",
        ...
        recommendedNextStep: passed
          ...
          : "Inspect model runtime health and rerun verification.",
        category: "remote_executor",
    ...
    checks.push(
      buildCheck({
        ...
        recommendedNextStep: "Verify LLM provider model health and connectivity.",
        category: "remote_executor",
        metadata: { failureCategory: classifyFetchFailure(error) },
      })
    );
  ...
  if (!probeAction) {
    checks.push(
      buildCheck({
        ...
        recommendedNextStep: "Restore the currentAdmin.status registry entry before production verification.",
        category: "remote_executor",
      ...
      affectedCount > 0
        ? [
            ...
            outcomeSamples?.length
              ? `Outcomes: ${formatSampleRows(outcomeSamples, (row) => `#${row.id} ${row.policy_reference}: ${normalizeString(row.outcome_summary) || "<missing summary>"}`)}`
              : "",
          ]
            .filter(Boolean)
            .join(" | ")
...
async function verifySourceJoinOrphansCheck(db) {
  ...
  const [outcomeSamples] = await db.query(
    `
      SELECT pos.policy_outcome_id, pos.source_id
      FROM policy_outcome_sources pos
      LEFT JOIN policy_outcomes po ON po.id = pos.policy_outcome_id
      LEFT JOIN sources s ON s.id = pos.source_id
      WHERE po.id IS NULL OR s.id IS NULL
      ORDER BY pos.policy_outcome_id ASC, pos.source_id ASC
      LIMIT 3
    `
  );
      ...
      affectedCount > 0
        ? [
            actionSamples?.length
              ...
              : "",
            outcomeSamples?.length
              ? `Outcome joins: ${formatSampleRows(outcomeSamples, (row) => `policy_outcome=${row.policy_outcome_id} source=${row.source_id}`)}`
              : "",
          ]
            .filter(Boolean)
            .join(" | ")
    ...
    SELECT
      ...
      SUM(
        CASE
          ...
          OR EXISTS (
            SELECT 1
            FROM policy_outcomes po2
            JOIN policy_outcome_sources pos2 ON pos2.policy_outcome_id = po2.id
            WHERE po2.policy_type = po.policy_type
              AND po2.policy_id = po.policy_id
              AND po2.id <> po.id
          )
          OR (
            ...
            AND (
              EXISTS (
                SELECT 1
                FROM promise_sources ps
                WHERE ps.promise_id = po.policy_id
              )
              OR EXISTS (
                SELECT 1
                FROM promise_actions pa
                JOIN promise_action_sources pas ON pas.promise_action_id = pa.id
                WHERE pa.promise_id = po.policy_id
              )
            )
          )
          THEN 1
