    ...
    details:
      duplicateGroups > 0
        ? `Sample clusters: ${formatSampleRows(
            sampleRows,
            (row) =>
              `${normalizeString(row.source_url)} (${row.duplicate_count} rows; titles=${row.distinct_titles}, types=${row.distinct_types}, publishers=${row.distinct_publishers}, dates=${row.distinct_published_dates})`
          )}`
        : "The source table is not carrying duplicate URL clusters right now.",
    recommendedNextStep:
      duplicateGroups > 0
        ? "Only canonicalize clusters where title, type, publisher, and published date match exactly; leave the rest for manual review."
