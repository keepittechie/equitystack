  ...
  if (item.workflowFamily === "current-admin" && item.queueType === "operator-review") {
    return [
      buildDescriptor({
        ...
        label: "Start Review",
        context,
        helperText: "Refreshes the canonical manual-review slice and decision-log sync path for this session.",
      }),
    ].filter(Boolean);
  }
  ...
  if (item.workflowFamily === "current-admin" && item.queueType === "apply-readiness") {
    if (item.state === "blocked") {
      const blockedActionId =
        item.recommendedActionId === "currentAdmin.review"
          ? "currentAdmin.review"
          : "currentAdmin.status";
      return [
        buildDescriptor({
          ...
          label:
            blockedActionId === "currentAdmin.review"
              ? "Open Review"
              : "Inspect Blocker",
          context,
          helperText:
            blockedActionId === "currentAdmin.review"
              ? "Resolve decision-log or manual-review blockers before rerunning apply."
              : "Read the canonical blocker state before rerunning apply.",
        }),
      ].filter(Boolean);
    }
      ...
      buildDescriptor({
        ...
        helperText:
          session.workflowFamily === "current-admin"
            ? "Reads the canonical current-admin pipeline state."
