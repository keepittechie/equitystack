...
const WORKFLOW_REFRESH_FAMILIES = new Set(["current-admin", "legislative"]);

function supportsWorkflowRefresh(action) {
  return WORKFLOW_REFRESH_FAMILIES.has(action?.workflowFamily);
}
...
function getNextSafeActionId({ action, sessionDetail, status }) {
  if (!supportsWorkflowRefresh(action)) {
    return action?.recommendedFollowUpActionId || null;
  }
...
function summarizeRun({ action, input, sessionDetail, artifacts }) {
  ...
  if (!supportsWorkflowRefresh(action)) {
    return `${action.title} completed.${artifactText}${fallbackText}`;
  }
...
export async function runRegisteredActionJob(jobId) {
  ...
  try {
    ...
    const normalizedInput = resolvedContext.normalizedInput;
    const before = supportsWorkflowRefresh(action)
      ? await captureWorkflowFamilyArtifactSnapshot(action.workflowFamily)
      : { session: null, workspace: null, artifacts: [], reviewQueue: [] };
    ...
    if (supportsWorkflowRefresh(action)) {
      ...
      sessionDetail = { session: null, workspace: null, artifacts: [], reviewQueue: [] };
    }
