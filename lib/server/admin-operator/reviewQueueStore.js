...
function normalizeReviewQueueItem(payload) {
  ...
  return {
    ...
    sourceArtifactId: toBoundedRecordId(payload.sourceArtifactId, "artifact") || null,
...
async function deleteReviewQueueRecordsFile(workflowFamily) {
  const records = await listReviewQueueRecordsFile({ workflowFamily, activeOnly: false });
  await Promise.all(
    records.map(async (record) => {
      await fs.rm(buildReviewQueueItemPath(record.id), { force: true }).catch(() => {});
    })
  );
}

export async function deleteReviewQueueRecordsByWorkflowFamily(workflowFamily) {
  const normalizedFamily = normalizeString(workflowFamily);
  if (!normalizedFamily) {
    return;
  }
  ...
  await withOperatorDbFallback(
    async (db) => {
      await db.execute(`DELETE FROM operator_review_queue_items WHERE workflow_family = ?`, [normalizedFamily]);
    },
    async () => deleteReviewQueueRecordsFile(normalizedFamily)
  );

  await deleteReviewQueueRecordsFile(normalizedFamily);
}
