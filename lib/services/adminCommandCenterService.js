...
function buildCurrentAdminSummary(workspace) {
  ...
  if (pendingReview > 0) {
    ...
    nextHref = "/admin/import-history";
    nextLabel = "Review latest import status";
  }
...
function buildWhatNowSummary({
  ...
  if (stalled) {
    return {
      ...
      next_step_label: "Run status summary in Operator Console",
    ...
    checks: [
      ...
      },
      {
        ...
        detail: historyExists
          ...
          : "No operator history exists yet. Use the Operator Console for a safe status action to initialize activity history.",
      },
      {
        id: "recommendations",
