...
function buildFallbackWorkflowOutcomeSummary({
  ...
  const decisions = {
    ...
    manual_review: Math.max(
      toSafeNumber(aiReview?.summary?.review_manually_count, 0),
      manualReviewCount
    ),
  };
  ...
  } else if (decisions.manual_review > 0) {
    workflowStatus = "completed_with_manual_review";
    runStatus = "success";
    confidenceLevel = "medium";
    userMessage = "AI review completed, but some items still require manual review.";
  } else {
...
function buildLegislativeWorkflowOutcomeSummary({
  ...
  const manualQueueCount = Math.max(
    manualReviewCount,
    toSafeNumber(manualQueue?.manual_review_count, 0),
    Array.isArray(manualQueue?.items) ? manualQueue.items.length : 0
  );
...
function buildLegislativeBlockers({
  ...
  if (manualReviewCount > 0) {
    blockers.push(
      `${manualReviewCount} legislative item(s) remain in the manual-review queue.`
    );
  }
...
export async function getLegislativeWorkflowWorkspace() {
  ...
  const manualQueueCount = Math.max(
    toSafeNumber(bundleSummary.manual_review_items, 0),
    toSafeNumber(manualQueue?.manual_review_count, 0),
    Array.isArray(manualQueue?.items) ? manualQueue.items.length : 0
  );
  ...
  return {
    ...
    counts: {
      ...
      pending_review_total: manualQueueCount + pendingUnreviewedActions.length,
          ...
          summary:
            key === "ai_review" && reviewRuntime
              ...
              : key === "manual_review_queue"
                ? manualQueueCount > 0
                  ? `${manualQueueCount} pending manual-review item(s)`
