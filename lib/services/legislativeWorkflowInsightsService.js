...
function summarizeOperatorActions(operatorActions) {
  ...
  return {
    ...
    autoApprovedActionableActions: approvedPendingActions,
...
function buildRepairReportSummary(repairReport, stat) {
  ...
  return {
    path: ARTIFACT_PATHS.repair_report,
    generated_at:
    ...
    actions_removed_or_resolved: actionsRemovedOrResolved,
    warnings: Array.isArray(repairReport.warnings) ? repairReport.warnings : [],
  };
}
...
export function buildLegislativeWorkflowOutcomeSummary({
  ...
  return {
    ...
    pending_bundle_approvals: pendingUnreviewedActions.length,
    approved_bundle_actions: approvedPendingActions.length,
    next_step_message: noActionableFollowup
...
export function buildLegislativeActionPermissions({
  ...
  return {
    ...
    run_apply_dry_run: buildActionPermission(
      hasBundle && !hasPendingHumanDecisions && hasApprovedActions,
      applyDryRunReasons
    ),
    apply_bundle: buildActionPermission(
      hasBundle &&
        ...
        hasApprovedActions &&
        hasCurrentApplyPreview &&
        (applyReport?.error_count || 0) === 0,
      applyReasons
    ),
...
export async function getLegislativeWorkflowWorkspace() {
  ...
  return {
    ...
    counts: {
      ...
      auto_approved_operator_actions: approvedPendingActions.length,
