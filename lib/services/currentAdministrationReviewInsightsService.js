...
function buildBatchStage(reviewReport, decisionTemplate, precommitReport) {
  if (!reviewReport) {
    return "review";
  }
  const items = decisionTemplate?.items || [];
  const hasAnyDecision = items.some((item) => normalizeString(item.operator_action));
  if (!hasAnyDecision) {
    return "decision";
  }
  if (!precommitReport) {
  ...
  if (precommitReport.readiness_status === "blocked") {
    return "blocked";
  }
  return "ready";
}
...
function buildWorkflowGuidePayload(reviewReports, decisionLogs, precommitReports) {
  ...
  return {
    canonical_process: {
      ...
      dashboard_role:
        "Read-only analytics and admin review surface; not a second review-generation or import pipeline.",
    },
    latest_review: latestReview
      ? {
          file_path: latestReview.file_path,
...
export async function getCurrentAdministrationOperatorWorkspace() {
  ...
  const stage = buildBatchStage(latestReview, matchingTemplate, matchingPrecommit);
      ...
      ? {
          ...
          paths: {
            ...
            decision_template:
              ...
              path.join(CURRENT_ADMIN_REPORTS_DIR, `${batchName}.decision-template.json`),
            queue: path.join(CURRENT_ADMIN_REPORTS_DIR, `${batchName}.manual-review-queue.json`),
            pre_commit:
