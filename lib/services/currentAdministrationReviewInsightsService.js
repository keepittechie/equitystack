...
const CURRENT_ADMIN_AUTOMATION_ARTIFACT_DEFINITIONS = [
  { key: "automation_report", suffix: "automation-report", label: "Automation report" },
  { key: "exception_queue", suffix: "exception-queue", label: "Exception queue" },
  { key: "outcome_evidence", suffix: "outcome-evidence", label: "Outcome evidence" },
  { key: "impact_evaluate", suffix: "impact-evaluate", label: "Impact evaluate" },
  { key: "impact_promote_dry_run", suffix: "impact-promote-dry-run", label: "Impact promote dry-run" },
  { key: "impact_promote_apply", suffix: "impact-promote-apply", label: "Impact promote apply" },
  {
    key: "outcome_enrichment_preview",
    suffix: "outcome-enrichment-preview",
    label: "Outcome enrichment preview",
  },
  {
    key: "outcome_enrichment_dry_run",
    suffix: "outcome-enrichment-dry-run",
    label: "Outcome enrichment dry-run",
  },
  {
    key: "outcome_enrichment_apply",
    suffix: "outcome-enrichment-apply",
    label: "Outcome enrichment apply",
  },
  {
    key: "outcome_sync_apply",
    suffix: "current-admin-outcome-sync-apply",
    label: "Outcome sync apply",
  },
];

export const CURRENT_ADMIN_AUTOMATION_ARTIFACT_KEYS =
  CURRENT_ADMIN_AUTOMATION_ARTIFACT_DEFINITIONS.map((entry) => entry.key);

const CURRENT_ADMIN_AUTOMATION_STAGE_DEFINITIONS = [
  { key: "discovery", label: "Discovery" },
  { key: "normalize", label: "Normalize" },
  { key: "ai_review", label: "AI review" },
  { key: "queue_promotion", label: "Queue promotion" },
  { key: "guarded_import", label: "Guarded import" },
  { key: "outcome_evidence", label: "Outcome evidence" },
  { key: "impact_evaluate", label: "Impact evaluate" },
  { key: "impact_promote", label: "Impact promote" },
  { key: "enrichment", label: "Enrichment" },
  { key: "outcome_sync", label: "Outcome sync" },
];
...
const OPERATOR_FRICTION_FORMULA = {
  ...
  inputs: [
    ...
    "operator_action in hold_for_evidence, reject_slogan, needs_more_sources, defer, reject, escalate (+1 when available)",
  ],
};
...
const DEFAULT_MANUAL_REVIEW_ACTIONS = [
  "publish_promise_only",
  "publish_action",
  "hold_for_evidence",
  "reject_slogan",
  "merge_existing_action",
  "link_existing_promise",
];
const ACTIVE_MANUAL_REVIEW_OPERATOR_ACTIONS = new Set([]);
const STRUCTURED_EDIT_PAYLOAD_KEYS = [
...
function hasStructuredEditPayload(value) {
  if (!value || typeof value !== "object") {
    return false;
  }

  for (const key of STRUCTURED_EDIT_PAYLOAD_KEYS) {
...
function availableOperatorActionsForItem(value) {
  const configured = Array.isArray(value?.available_operator_actions)
    ? value.available_operator_actions
  ...
  if (hasStructuredEditPayload(value)) {
    return ["approve_with_changes", ...DEFAULT_MANUAL_REVIEW_ACTIONS];
  }
  return [...DEFAULT_MANUAL_REVIEW_ACTIONS];
}

function isActiveManualReviewAction(operatorAction) {
  const normalized = normalizeString(operatorAction);
  if (!normalized) {
    return true;
  }
  return ACTIVE_MANUAL_REVIEW_OPERATOR_ACTIONS.has(normalized);
}
...
function manualReviewRecommendedActionFromSuggestion(suggestion = {}) {
  if (manualReviewSafeAutoResolutionFromSuggestion(suggestion)) {
    return "approve";
  }
...
function manualReviewSafeAutoResolutionFromSuggestion(suggestion = {}) {
  if (suggestion.safe_auto_resolution === true) {
    return true;
  }
  return ["no_material_change", "source_only_refresh"].includes(
    normalizeString(suggestion.existing_update_resolution)
  );
}
...
function manualReviewReasonLabels({ context, validation, item }) {
  ...
  if (manualReviewSafeAutoResolutionFromSuggestion(item.suggestions || {}) && !labels.length) {
    return [];
  }
  if (
    ...
    for (const item of template.items || []) {
      ...
      if (slug && operatorAction) {
        actions.set(slug, {
          ...
          has_structured_edit_payload: hasStructuredEditPayload(item),
          available_operator_actions: availableOperatorActionsForItem(item),
        });
      }
    }
  }
...
async function buildCurrentAdminManualReviewState({
  ...
  for (const [index, item] of (reviewReport.items || []).entries()) {
    ...
    if (action.operator_action && !isActiveManualReviewAction(action.operator_action)) {
      continue;
    }
    ...
    items.push({
      ...
      has_structured_edit_payload: Boolean(action.has_structured_edit_payload),
      available_operator_actions:
        action.available_operator_actions || availableOperatorActionsForItem(action),
...
async function listAutomationReportFiles() {
  return listJsonFiles(
    CURRENT_ADMIN_REPORTS_DIR,
    (name) => name.endsWith(".automation-report.json")
  );
}

function currentAdminAutomationArtifactPath(batchName, suffix) {
  return path.join(CURRENT_ADMIN_REPORTS_DIR, `${batchName}.${suffix}.json`);
}

function resolveCurrentAdminAutomationArtifactPath(batchName, artifactKey) {
  const batch = normalizeString(batchName);
  const definition = CURRENT_ADMIN_AUTOMATION_ARTIFACT_DEFINITIONS.find(
    (entry) => entry.key === normalizeString(artifactKey)
  );
  if (!batch || !definition) {
    return null;
  }
  return currentAdminAutomationArtifactPath(batch, definition.suffix);
}

function normalizeAutomationStage(value) {
  return normalizeString(value)
    .toLowerCase()
    .replace(/[-\s]+/g, "_");
}

function formatAutomationStageLabel(value) {
  const normalized = normalizeAutomationStage(value);
  if (!normalized) {
    return "Unknown";
  }
  const definition = CURRENT_ADMIN_AUTOMATION_STAGE_DEFINITIONS.find(
...
function currentAdminTimelineBlockedStage(exceptionStage) {
  switch (normalizeAutomationStage(exceptionStage)) {
    case "manual_review":
      return "queue_promotion";
    case "apply_blocked":
      return "guarded_import";
    case "impact_blocked":
      return "impact_evaluate";
    case "enrichment_blocked":
      return "enrichment";
    default:
      return "";
  }
...
function currentAdminTimelineFurthestStage(reportStage, reportStatus, artifactStatus = {}) {
  const normalizedStage = normalizeAutomationStage(reportStage);
  const normalizedStatus = normalizeString(reportStatus).toLowerCase();

  if (normalizedStatus === "complete" || normalizedStatus === "dry_run_complete") {
    return artifactStatus.outcome_sync_apply?.exists ? "outcome_sync" : "enrichment";
  }
  ...
  switch (normalizedStage) {
    ...
    case "apply":
      return "guarded_import";
    case "impact_evaluate":
      return "impact_evaluate";
    case "impact_promote":
      return "impact_promote";
    case "enrichment":
      return artifactStatus.outcome_sync_apply?.exists ? "outcome_sync" : "enrichment";
    default:
      return "";
...
function summarizeAutomationArtifactPayload(artifactKey, payload = null) {
  if (!payload || typeof payload !== "object") {
    return null;
  }

  if (artifactKey === "automation_report") {
    const status = normalizeString(payload.status);
    const stage = formatAutomationStageLabel(payload.current_stage);
    return [status, stage].filter(Boolean).join(" • ") || null;
  }
  if (artifactKey === "exception_queue") {
    const count = Array.isArray(payload.items) ? payload.items.length : 0;
    const stage = formatAutomationStageLabel(payload.stage);
    return `${count} item(s) • ${stage}`;
  }
  if (artifactKey === "outcome_evidence") {
    const summary = payload.summary || {};
    return [
      `${toSafeNumber(summary.records_scanned, 0)} scanned`,
      `${toSafeNumber(summary.strong_matches, 0)} strong`,
      `${toSafeNumber(summary.review_matches, 0)} review`,
      `${toSafeNumber(summary.weak_matches, 0)} weak`,
    ].join(" • ");
  }
  if (artifactKey === "impact_evaluate") {
    const counts = payload.counts || {};
    return [
      `${toSafeNumber(counts.auto_approved_count, 0)} auto-approved`,
      `${toSafeNumber(counts.blocked_count, 0)} blocked`,
    ].join(" • ");
  }
  if (artifactKey === "impact_promote_dry_run" || artifactKey === "impact_promote_apply") {
    return `${toSafeNumber(payload.impact_promoted_count, 0)} promoted`;
  }
  if (
    artifactKey === "outcome_enrichment_preview" ||
    artifactKey === "outcome_enrichment_dry_run" ||
    artifactKey === "outcome_enrichment_apply"
  ) {
    const summary = payload.summary || {};
    return [
      `${toSafeNumber(summary.approved_count, 0)} approved`,
      `${toSafeNumber(summary.blocked_count, 0)} blocked`,
    ].join(" • ");
  }
  if (artifactKey === "outcome_sync_apply") {
    return normalizeString(payload.message) || normalizeString(payload.status) || null;
  }
  return null;
}

async function loadCurrentAdminAutomationArtifacts(batchName) {
  const batch = normalizeString(batchName);
  if (!batch) {
    return {};
  }
  ...
  const entries = await Promise.all(
    CURRENT_ADMIN_AUTOMATION_ARTIFACT_DEFINITIONS.map(async (definition) => {
      const filePath = currentAdminAutomationArtifactPath(batch, definition.suffix);
      const stat = await statSafe(filePath);
      const payload = stat ? await readJsonFileOrNull(filePath) : null;
      return [
        definition.key,
        {
          ...definition,
          ...
          payload,
          summary: summarizeAutomationArtifactPayload(definition.key, payload),
        },
      ];
    })
  );
...
async function loadLatestCurrentAdminAutomationReport() {
  const files = await listAutomationReportFiles();
  if (!files.length) {
    return null;
  }

  const latest = files[0];
  const payload = await readJsonFileOrNull(latest.filePath);
  return {
    file_path: latest.filePath,
    generated_at: latest.stat?.mtime?.toISOString?.() || null,
    payload,
    batch_name:
      normalizeString(payload?.batch_name) ||
      path.basename(latest.filePath, ".automation-report.json"),
  };
}

function buildCurrentAdminAutomationArtifactStatus(automationArtifacts = {}) {
  return Object.fromEntries(
    CURRENT_ADMIN_AUTOMATION_ARTIFACT_DEFINITIONS.map((definition) => {
      const artifact = automationArtifacts[definition.key] || {};
      return [
        definition.key,
        buildArtifactEntry({
...
function buildCurrentAdminAutomationTimeline({ automationReport, exceptionQueue, artifactStatus }) {
  const reportStatus = normalizeString(automationReport?.status).toLowerCase();
  const blockedStageKey = currentAdminTimelineBlockedStage(exceptionQueue?.stage);
  const furthestStageKey = currentAdminTimelineFurthestStage(
    automationReport?.current_stage,
    automationReport?.status,
    artifactStatus
  );
  const furthestIndex = CURRENT_ADMIN_AUTOMATION_STAGE_DEFINITIONS.findIndex(
    (entry) => entry.key === furthestStageKey
  );
  const noImportableItems = normalizeString(automationReport?.message)
    .toLowerCase()
    .includes("no importable queue items remained");

  return CURRENT_ADMIN_AUTOMATION_STAGE_DEFINITIONS.map((stage, index) => {
    let status = "pending";
    let detail = "";
    if (stage.key === blockedStageKey) {
      status = "blocked";
      detail = normalizeString(exceptionQueue?.reason) || normalizeString(automationReport?.message);
    } else if (reportStatus === "failed" && index === furthestIndex) {
      status = "failed";
      detail = normalizeString(automationReport?.message);
    } else if (furthestIndex >= 0 && index <= furthestIndex) {
      status = "passed";
    }

    if (stage.key === "guarded_import" && noImportableItems) {
      status = "skipped";
      detail = "No importable queue items remained after AI-first review.";
    }
    if (
      stage.key === "impact_promote" &&
      !artifactStatus.impact_promote_dry_run?.exists &&
      !artifactStatus.impact_promote_apply?.exists &&
      artifactStatus.impact_evaluate?.exists &&
      furthestIndex >= CURRENT_ADMIN_AUTOMATION_STAGE_DEFINITIONS.findIndex((entry) => entry.key === "impact_evaluate")
    ) {
      status = "skipped";
      detail = "No validator-approved impact transitions were available to promote.";
    }
    if (
      stage.key === "outcome_sync" &&
      !artifactStatus.outcome_sync_apply?.exists &&
      (reportStatus === "complete" || reportStatus === "dry_run_complete" || furthestStageKey === "enrichment")
    ) {
      status = "skipped";
      detail =
        reportStatus === "dry_run_complete"
          ? "Read-only automation skipped mutating sync steps."
          : "No approved enrichment apply generated outcome-sync work.";
    }

    if (status === "passed" && reportStatus === "stopped" && stage.key === furthestStageKey) {
      detail = "Run stopped intentionally at this checkpoint.";
    }
...
function buildCurrentAdminAutomationState({
  batchName,
  ...
  const batch = normalizeString(batchName);
  const automationReportArtifact = automationArtifacts.automation_report || {};
  const exceptionQueueArtifact = automationArtifacts.exception_queue || {};
  const outcomeEvidenceArtifact = automationArtifacts.outcome_evidence || {};
  const impactEvaluateArtifact = automationArtifacts.impact_evaluate || {};
  const impactPromoteArtifact =
    automationArtifacts.impact_promote_apply?.exists
      ? automationArtifacts.impact_promote_apply
      : automationArtifacts.impact_promote_dry_run || {};
  const enrichmentArtifact =
    automationArtifacts.outcome_enrichment_apply?.exists
      ? automationArtifacts.outcome_enrichment_apply
      : automationArtifacts.outcome_enrichment_dry_run?.exists
        ? automationArtifacts.outcome_enrichment_dry_run
        : automationArtifacts.outcome_enrichment_preview || {};

  const automationReport = automationReportArtifact.payload || latestAutomationReport?.payload || null;
  const exceptionQueue = exceptionQueueArtifact.payload || null;
  const outcomeEvidence = outcomeEvidenceArtifact.payload || null;
  const impactEvaluate = impactEvaluateArtifact.payload || null;
  const impactPromote = impactPromoteArtifact.payload || null;
  const enrichment = enrichmentArtifact.payload || null;

  const evidenceSummary = outcomeEvidence?.summary || {};
  const impactCounts = impactEvaluate?.counts || {};
  const enrichmentSummary = enrichment?.summary || {};
  const exceptionItems = Array.isArray(exceptionQueue?.items) ? exceptionQueue.items : [];

  const viewerArtifacts = CURRENT_ADMIN_AUTOMATION_ARTIFACT_DEFINITIONS.map((definition) => {
    const artifact = automationArtifacts[definition.key] || {};
    return {
      key: definition.key,
      label: definition.label,
  ...
  return {
    available: Boolean(batch && (automationReport || viewerArtifacts.some((artifact) => artifact.exists))),
    batch_name: batch,
    latest_run: {
      status: normalizeString(automationReport?.status) || "not_started",
      current_stage: normalizeAutomationStage(automationReport?.current_stage) || "pending",
      current_stage_label: formatAutomationStageLabel(automationReport?.current_stage) || "Pending",
      message: normalizeString(automationReport?.message) || "No current-admin automation report is available yet.",
      generated_at:
        normalizeString(automationReport?.generated_at) ||
        automationReportArtifact.generated_at ||
        latestAutomationReport?.generated_at ||
        null,
      artifact_path: automationReportArtifact.path || latestAutomationReport?.file_path || null,
    },
    summary: {
      exception_count: exceptionItems.length,
      impact_auto_approved_count: toSafeNumber(impactCounts.auto_approved_count, 0),
      impact_blocked_count: toSafeNumber(impactCounts.blocked_count, 0),
      promoted_count: toSafeNumber(impactPromote?.impact_promoted_count, 0),
      enrichment_approved_count: toSafeNumber(enrichmentSummary.approved_count, 0),
      enrichment_blocked_count: toSafeNumber(enrichmentSummary.blocked_count, 0),
      records_scanned: toSafeNumber(evidenceSummary.records_scanned, 0),
      outcome_evidence_matches: toSafeNumber(evidenceSummary.outcome_evidence_matches, 0),
      implementation_evidence_matches: toSafeNumber(evidenceSummary.implementation_evidence_matches, 0),
      strong_matches: toSafeNumber(evidenceSummary.strong_matches, 0),
      review_matches: toSafeNumber(evidenceSummary.review_matches, 0),
      weak_matches: toSafeNumber(evidenceSummary.weak_matches, 0),
    },
    exception_queue: {
      present: Boolean(exceptionQueueArtifact.exists),
      stage: normalizeAutomationStage(exceptionQueue?.stage) || "resolved",
      stage_label: formatAutomationStageLabel(exceptionQueue?.stage) || "Resolved",
      reason: normalizeString(exceptionQueue?.reason) || "",
      next_step: normalizeString(exceptionQueue?.next_step) || "",
      recommended_command: normalizeString(exceptionQueue?.recommended_command) || "",
      source_artifact: normalizeString(exceptionQueue?.source_artifact) || "",
      artifact_path: exceptionQueueArtifact.path || null,
      item_count: exceptionItems.length,
      items: exceptionItems.slice(0, 25),
    },
    stages: buildCurrentAdminAutomationTimeline({
      automationReport,
      exceptionQueue,
      artifactStatus,
    }),
    viewer_artifacts: viewerArtifacts,
    default_viewer_artifact_key:
      viewerArtifacts.find((artifact) => artifact.key === "automation_report" && artifact.exists)?.key ||
      viewerArtifacts.find((artifact) => artifact.key === "exception_queue" && artifact.exists)?.key ||
      viewerArtifacts.find((artifact) => artifact.exists)?.key ||
      "automation_report",
  };
}

export async function readCurrentAdminAutomationArtifact({
  batchName,
  artifactKey,
}) {
  const batch = normalizeString(batchName);
  const key = normalizeString(artifactKey);
  if (!batch) {
    throw new Error("A current-admin batch name is required.");
  }
  if (!CURRENT_ADMIN_AUTOMATION_ARTIFACT_KEYS.includes(key)) {
    throw new Error(`Unsupported current-admin automation artifact: ${key}`);
  }

  const filePath = resolveCurrentAdminAutomationArtifactPath(batch, key);
  if (!filePath) {
    throw new Error(`Could not resolve the ${key} artifact path.`);
  }
  ...
  if (!stat) {
    return {
      ...
      artifact_key: key,
      label:
        CURRENT_ADMIN_AUTOMATION_ARTIFACT_DEFINITIONS.find((entry) => entry.key === key)?.label ||
        key,
      exists: false,
      path: filePath,
      generated_at: null,
  ...
  return {
    ...
    path: filePath,
    generated_at: stat.mtime?.toISOString?.() || null,
    payload: await readJsonFileOrNull(filePath),
  };
}
...
function buildWorkspaceReviewItems(reviewReport, decisionTemplate, latestDecision, queuePayload) {
  ...
  const queueBySlug = new Map(
    [
      ...queueManualItems(queuePayload),
      ...queueAutoApprovedItems(queuePayload),
      ...queueAutoRejectedItems(queuePayload),
    ]
      .filter((item) => normalizeString(item?.slug))
    ...
    ? new Set(
        queueManualItems(queuePayload)
          .filter((item) => {
            ...
            return !operatorStatus || ["pending", "pending_manual_review"].includes(operatorStatus);
          })
          .map((item) => normalizeString(item.slug))
          .filter(Boolean)
      )
  ...
  return sourceItems.map((item, index) => {
    ...
    const queueItem = queueBySlug.get(normalizeString(item.slug));
    const finalRecord = queueItem?.final_record || null;
    const firstAction =
      finalRecord && Array.isArray(finalRecord.actions) ? finalRecord.actions[0] || null : null;
    const operatorAction =
      normalizeString(templateItem?.operator_action) ||
      normalizeString(lastDecision?.operator_action) ||
      "";
    if (!isActiveManualReviewAction(operatorAction)) {
      return null;
    }
    ...
    return {
      ...
      entity_disposition:
        normalizeString(queueItem?.entity_disposition) ||
        normalizeString(finalRecord?.entity_disposition) ||
        "",
      mechanism_type:
        normalizeString(queueItem?.mechanism_type) ||
        normalizeString(firstAction?.governing_action_type) ||
        normalizeString(firstAction?.action_type) ||
        "",
      score_eligibility:
        normalizeString(queueItem?.score_eligibility) ||
        normalizeString(firstAction?.score_eligibility) ||
        normalizeString(finalRecord?.score_eligibility) ||
        "",
      visibility_state:
        normalizeString(queueItem?.visibility_state) ||
        normalizeString(firstAction?.visibility_state) ||
        normalizeString(finalRecord?.visibility_state) ||
        "",
      reject_reason_code:
        normalizeString(queueItem?.reject_reason_code) ||
        normalizeString(finalRecord?.reject_reason_code) ||
        "",
      candidate_dedupe_key:
        normalizeString(queueItem?.candidate_dedupe_key) ||
        normalizeString(firstAction?.dedupe_key) ||
        "",
      likely_duplicate_action_id:
        queueItem?.likely_duplicate_action_id ||
        finalRecord?.likely_duplicate_action_id ||
        null,
      has_structured_edit_payload:
        Boolean(templateItem?.has_structured_edit_payload) ||
        Boolean(lastDecision?.has_structured_edit_payload),
      available_operator_actions:
        availableOperatorActionsForItem(templateItem || lastDecision || {}),
    };
  }).filter(Boolean);
}
...
function buildWorkflowGuidePayload(reviewReports, decisionLogs, precommitReports) {
  ...
  const nextStepLabels = {
    start_or_resume_review: "Run or inspect the canonical current-admin pipeline",
    review_manual_slice: "Promote AI decisions and review only the remaining manual slice",
  ...
  if (latestReview && !latestDecision) {
    nextStep = "review_manual_slice";
    nextCommands = [
      `./bin/equitystack current-admin review --input ${path.relative(PYTHON_DIR, latestReview.file_path)}`,
  ...
  return {
    ...
    workflow_handoff: {
      ...
      operator_notes: [
        "Use current-admin review to refresh the canonical manual-review slice before filling operator actions.",
        "Log operator decisions explicitly before import so AI-vs-operator outcomes remain auditable.",
        "Use current-admin deep-review when the batch or a flagged row needs a deeper paired AI pass before operator judgment.",
...
export async function getCurrentAdministrationOperatorWorkspace() {
  ...
  const latestAutomationReport = await loadLatestCurrentAdminAutomationReport();
  ...
  const automationBatchName = batchName || latestAutomationReport?.batch_name || null;
  ...
  const automationArtifacts = await loadCurrentAdminAutomationArtifacts(automationBatchName);
  const artifactStatus = {
    ...baseArtifactStatus,
    ...buildCurrentAdminAutomationArtifactStatus(automationArtifacts),
  };
  const automationState = buildCurrentAdminAutomationState({
