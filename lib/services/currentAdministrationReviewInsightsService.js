...
function buildCurrentAdminImportReadiness(queuePayload, precommitReport, reviewCounts) {
  ...
  const queueApprovedForImportCount = queueItems.filter(
    (item) =>
      item &&
      typeof item === "object" &&
      (Boolean(item.approved) || normalizeString(item.operator_status) === "approved")
  ).length;
...
function buildCurrentAdminQueuePromotionState(queuePayload) {
  ...
  return {
    ...
    non_promoted_item_count: toSafeNumber(promotionState.non_promoted_item_count, 0),
    impact_outcomes_deferred: Boolean(promotionState.impact_outcomes_deferred),
    status_label: "Promoted Review Queue",
    operator_hint: `${toSafeNumber(
      promotionState.approved_item_count,
      0
    )} impact-pending item(s) approved for import; finalized impact outcomes remain deferred.`,
  };
}
...
export async function getCurrentAdministrationOperatorWorkspace() {
  ...
  const artifactStatus = {
    ...
    manual_review_queue: buildArtifactEntry({
      ...
      stage: "QUEUE_READY",
      summary: queueStat
        ? `${importReadiness.queue_approved_for_import_count} approved for import • ${importReadiness.queue_pending_manual_review_count} held for review • ${importReadiness.queue_pending_count} pending`
        : null,
    }),
