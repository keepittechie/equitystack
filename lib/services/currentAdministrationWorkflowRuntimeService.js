...
function buildDecisionLogPath(reviewPath) {
  const batchName = deriveBatchNameFromReviewPath(reviewPath);
  return path.join(currentAdminReportsDir, "review_decisions", `${batchName}.decision-log.json`);
}
...
function assertPermission(workspace, permissionKey) {
  ...
  if (!permission?.allowed) {
    const reason = permission?.reasons?.[0] || "The requested action is blocked by the canonical current-admin pipeline state.";
...
export async function saveCurrentAdministrationDecisionDraft({
  ...
  if (!templatePayload || !templateMatchesReview) {
    await runPythonCommand([
      "scripts/promote_current_admin_review_to_queue.py",
      "--input",
      resolvedReviewPath,
      "--decision-template-output",
      outputPath,
      "--decision-log-output",
      buildDecisionLogPath(resolvedReviewPath),
      "--no-backup",
    ]);
