...
export async function fetchHomepageReadinessSummary() {
  const [outcomeRows, duplicateRows, policyRows] = await Promise.all([
    query(`
      SELECT
        ...
        SUM(policy_type = 'governing_action') AS governing_action_outcomes,
        ...
        SUM(
          ...
          OR policy_type NOT IN ('current_admin', 'legislative', 'judicial_impact', 'governing_action')
  ...
  const governingActionOutcomes = toNumber(outcomeSummary.governing_action_outcomes);
  ...
  return {
    ...
    governing_action_outcomes: governingActionOutcomes,
