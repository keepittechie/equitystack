import { getCurrentAdminReadinessActionMetaBySlug } from "@/lib/currentAdmin/reportArtifacts";
...
import { computeOutcomeScore } from "@/lib/black-impact-score/outcomeScoring";
import { computePromiseFromOutcomes } from "@/lib/black-impact-score/promiseRollup";
import {
...
function isPublicCurrentAdminPromiseRow(row = {}) {
  if (resolvePromisePresidentSlug(row) !== CURRENT_ADMINISTRATION_PROMISE_SLUG) {
    return true;
  }
  return ["public_unscored", "public_scored"].includes(
    deriveCurrentAdminPromiseVisibilityState(row)
  );
}
...
function getPromiseOutcomeSourceCount(outcome = {}) {
  return (
    toSafeCount(outcome.unified_source_count) ||
    toSafeCount(outcome.outcome_sources?.length) ||
    toSafeCount(outcome.sources?.length) ||
    0
  );
}

function buildPromiseBlackImpactSummary(promise = {}, outcomes = []) {
  const scoredOutcomes = outcomes
    .map((outcome) =>
      computeOutcomeScore({
        outcome: {
          ...outcome,
          source_count: getPromiseOutcomeSourceCount(outcome),
        },
        source_count: getPromiseOutcomeSourceCount(outcome),
    ...
    .filter((entry) => entry?.factors?.scoring_ready === true);

  if (!scoredOutcomes.length) {
    return null;
  }

  const summary = computePromiseFromOutcomes({
    promise,
    scored_outcomes: scoredOutcomes,
  });

  const direction =
    summary.total_score > 0 ? "Positive" : summary.total_score < 0 ? "Negative" : "Mixed";

  return {
    total_score: summary.total_score,
    direction,
    direction_label:
      direction === "Mixed" ? "Mixed or limited" : `${direction} impact`,
    outcome_count: summary.outcome_count,
    confidence_label: summary.confidence_label || null,
    completeness_label: summary.completeness_label || null,
    explanation_summary: summary.explanation_summary || null,
    breakdown_by_direction: summary.breakdown_by_direction || {},
    trust_summary: summary.trust_summary || null,
    systemic_impact_category: summary.systemic_impact_category || null,
    systemic_explanation_summary: summary.systemic_explanation_summary || null,
  };
}

function mapCurrentAdminReadinessMeta(meta = null) {
  const nextReviewDate = normalizeString(meta?.next_review_date) || null;
  const dataRefreshTrigger = normalizeString(meta?.data_refresh_trigger) || null;

  return {
    scoring_ready: meta?.scoring_ready === true,
    readiness_recommendation: normalizeString(meta?.current_recommendation) || null,
    readiness_blockers: Array.isArray(meta?.blockers)
      ? meta.blockers.filter((item) => typeof item === "string" && item.trim())
      : [],
    next_review_date: nextReviewDate,
    data_refresh_trigger: dataRefreshTrigger,
    next_check_hint: buildCurrentAdminNextCheckHint({
      next_review_date: nextReviewDate,
      data_refresh_trigger: dataRefreshTrigger,
    }),
  };
...
async function getOutcomeSources(outcomeIds = []) {
  ...
  const rows = await query(
    ...
    WHERE pout.id IN (${outcomeIds.map(() => "?").join(", ")})
    ORDER BY pout.id ASC, s.published_date DESC, s.id ASC
    `,
    outcomeIds
  );
...
function buildGoverningActionSourcesMap(rows) {
  const grouped = new Map();

  for (const row of rows) {
    const key = row.governing_action_id;
    if (!grouped.has(key)) {
      grouped.set(key, []);
    }
...
async function getGoverningActionSources(actionIds = []) {
  if (!actionIds.length) {
    return new Map();
  }
  ...
  try {
    rows = await query(
      `
    SELECT
      gas.governing_action_id,
      COALESCE(s.id, gas.id) AS id,
      COALESCE(s.source_title, gas.source_title) AS title,
      COALESCE(s.publisher, gas.source_title) AS publisher,
      COALESCE(s.source_url, gas.source_url) AS url,
      COALESCE(s.source_type, gas.source_type) AS source_type,
      gas.source_quality,
      gas.evidence_role
    FROM governing_action_sources gas
    LEFT JOIN sources s ON s.id = gas.source_id
    WHERE gas.governing_action_id IN (${actionIds.map(() => "?").join(", ")})
    ORDER BY gas.governing_action_id ASC, gas.id ASC
      `,
      actionIds
    );
  } catch (error) {
  ...
  return buildGoverningActionSourcesMap(rows);
}

function buildGoverningActionCategoryMap(rows) {
  const grouped = new Map();
...
async function getGoverningActionCategories(actionIds = []) {
  if (!actionIds.length) {
    return new Map();
  }
  ...
  try {
    rows = await query(
      `
    SELECT
      gapc.governing_action_id,
      pc.slug,
      pc.name
    FROM governing_action_policy_categories gapc
    JOIN policy_categories pc ON pc.id = gapc.category_id
    WHERE gapc.governing_action_id IN (${actionIds.map(() => "?").join(", ")})
    ORDER BY gapc.governing_action_id ASC, pc.name ASC
      `,
      actionIds
    );
  } catch (error) {
  ...
  return buildGoverningActionCategoryMap(rows);
}

async function fetchGoverningActionRowsForPromiseIds(promiseIds = [], { publicOnly = false } = {}) {
  ...
  try {
    ...
    SELECT
      ...
      ga.action_date,
      ga.effective_date,
      ga.official_identifier,
      ga.parent_action_id,
      ga.chain_relationship_type,
      ga.best_source_quality,
      ga.visibility_state,
      ga.score_eligibility,
      ga.data_confidence,
      parent.slug AS parent_action_slug,
      parent.title AS parent_action_title,
      (
        SELECT COUNT(*)
        FROM governing_action_sources gas
        WHERE gas.governing_action_id = ga.id
      ) AS source_count,
      (
        SELECT COUNT(*)
        FROM policy_outcomes po
        WHERE po.policy_type = 'governing_action'
          AND po.policy_id = ga.id
      ) AS outcome_count,
      (
        SELECT COUNT(DISTINCT pga_link.promise_id)
        FROM promise_governing_actions pga_link
        WHERE pga_link.governing_action_id = ga.id
      ) AS linked_promise_count,
      (
        SELECT p.slug
        FROM promise_governing_actions pga_link
        JOIN promises p ON p.id = pga_link.promise_id
        WHERE pga_link.governing_action_id = ga.id
        ORDER BY p.id ASC
        LIMIT 1
      ) AS primary_promise_slug,
      (
        SELECT p.title
        FROM promise_governing_actions pga_link
        JOIN promises p ON p.id = pga_link.promise_id
        ...
        ORDER BY p.id ASC
        LIMIT 1
      ) AS primary_promise_title
    FROM promise_governing_actions pga
    JOIN governing_actions ga ON ga.id = pga.governing_action_id
    LEFT JOIN governing_actions parent ON parent.id = ga.parent_action_id
    WHERE pga.promise_id IN (${promiseIds.map(() => "?").join(", ")})
      ${publicOnly ? "AND ga.visibility_state IN ('public_unscored', 'public_scored')" : ""}
    ORDER BY COALESCE(ga.action_date, ga.effective_date) DESC, ga.id DESC
      `,
      promiseIds
    );
  } catch (error) {
  ...
  const [sourcesMap, categoriesMap, readinessMetaBySlug] = await Promise.all([
    getGoverningActionSources(actionIds),
    getGoverningActionCategories(actionIds),
    getCurrentAdminReadinessActionMetaBySlug(),
  ]);

  const grouped = new Map();
  for (const row of rows) {
    const promiseId = row.promise_id;
    if (!grouped.has(promiseId)) {
      grouped.set(promiseId, []);
    }
    grouped.get(promiseId).push({
      ...row,
      source_count: Number(row.source_count || 0),
      outcome_count: Number(row.outcome_count || 0),
      linked_promise_count: Number(row.linked_promise_count || 0),
      sources: sourcesMap.get(row.id) || [],
      categories: categoriesMap.get(row.id) || [],
      ...mapCurrentAdminReadinessMeta(readinessMetaBySlug.get(row.slug) || null),
    });
  }
...
async function fetchPublicGoverningActionsForPresident(presidentId, { limit = null } = {}) {
  let rows = [];
  try {
    rows = await query(
      `
    SELECT
      ...
      (
        SELECT COUNT(DISTINCT pga.promise_id)
        FROM promise_governing_actions pga
        WHERE pga.governing_action_id = ga.id
      ) AS linked_promise_count,
      (
        SELECT p.slug
        FROM promise_governing_actions pga_link
    ...
    FROM governing_actions ga
    LEFT JOIN governing_actions parent ON parent.id = ga.parent_action_id
    WHERE ga.president_id = ?
      AND ga.visibility_state IN ('public_unscored', 'public_scored')
    ORDER BY COALESCE(ga.action_date, ga.effective_date) DESC, ga.id DESC
    ${Number.isFinite(limit) ? `LIMIT ${Math.max(0, Number(limit))}` : ""}
      `,
      [presidentId]
    );
  } catch (error) {
...
function mapPromiseCardRow(row) {
  const visibility_state =
    resolvePromisePresidentSlug(row) === CURRENT_ADMINISTRATION_PROMISE_SLUG
      ? deriveCurrentAdminPromiseVisibilityState(row)
      : normalizeString(row.visibility_state) || null;
  return applyPromiseCuration({
    ...
    governing_action_count: Number(row.governing_action_count || 0),
    public_governing_action_count: Number(row.public_governing_action_count || 0),
    public_scored_governing_action_count: Number(
      row.public_scored_governing_action_count || 0
    ),
    outcome_count: Number(row.outcome_count || 0),
    source_count: Number(row.source_count || 0),
    promise_public_source_count: Number(row.promise_public_source_count || 0),
    related_policy_count: Number(row.related_policy_count || 0),
    related_explainer_count: Number(row.related_explainer_count || 0),
    visibility_state,
    tracked_but_not_scored:
      resolvePromisePresidentSlug(row) === CURRENT_ADMINISTRATION_PROMISE_SLUG
        ? isCurrentAdminPromiseTrackedButNotScored(row)
        : Number(row.public_governing_action_count || 0) === 0 &&
          Number(row.outcome_count || 0) === 0,
  });
}
...
function buildNormalizedPromiseStatusCounts(rows = []) {
  const counts = PROMISE_STATUSES.reduce((totals, status) => {
    totals[status] = 0;
    return totals;
  }, {});
...
async function fetchPromiseCardRows(whereClause = "", params = [], orderBy = "") {
  ...
  try {
    ...
    SELECT
      ...
      (
        SELECT COUNT(DISTINCT pga.governing_action_id)
        FROM promise_governing_actions pga
        WHERE pga.promise_id = p.id
      ) AS governing_action_count,
      (
        SELECT COUNT(DISTINCT pga.governing_action_id)
        FROM promise_governing_actions pga
        JOIN governing_actions ga ON ga.id = pga.governing_action_id
        WHERE pga.promise_id = p.id
          AND ga.visibility_state IN ('public_unscored', 'public_scored')
      ) AS public_governing_action_count,
      (
        SELECT COUNT(DISTINCT pga.governing_action_id)
        FROM promise_governing_actions pga
        JOIN governing_actions ga ON ga.id = pga.governing_action_id
        WHERE pga.promise_id = p.id
          AND ga.visibility_state = 'public_scored'
      ) AS public_scored_governing_action_count,
      (
        SELECT COUNT(*)
        FROM promise_outcomes pout
      ...
      (
        ...
        WHERE ps.promise_id = p.id
          AND ps.evidence_role = 'promise_attribution'
          AND ps.source_quality IN ('primary_official', 'secondary_verified')
      ) AS promise_public_source_count,
      (
        ...
        FROM (
          ...
          UNION
          SELECT pos.source_id, po.policy_id AS owner_promise_id
          FROM policy_outcomes po
          JOIN policy_outcome_sources pos
            ON pos.policy_outcome_id = po.id
      ...
      (
        SELECT MAX(COALESCE(ga.action_date, ga.effective_date))
        FROM promise_governing_actions pga
        JOIN governing_actions ga ON ga.id = pga.governing_action_id
        WHERE pga.promise_id = p.id
          AND ga.visibility_state IN ('public_unscored', 'public_scored')
      ) AS latest_governing_action_date,
      (
    ...
    rows = await query(
      ...
      SELECT
        ...
        0 AS governing_action_count,
        0 AS public_governing_action_count,
        0 AS public_scored_governing_action_count,
        (
          SELECT COUNT(*)
          FROM promise_outcomes pout
          WHERE pout.promise_id = p.id
        ...
        NULL AS latest_governing_action_date,
        (
          SELECT COUNT(DISTINCT pact_policy.related_policy_id)
          FROM promise_actions pact_policy
          WHERE pact_policy.promise_id = p.id
...
export async function fetchPromiseList({
  ...
  const [countRows, statusCountRows, rows] = await Promise.all([
    query(
    ...
    query(
      ...
      ${fromAndWhere}
      GROUP BY p.status
      ORDER BY ${PROMISE_STATUS_FIELD_SQL}
      `,
      params
    ),
    query(
  ...
  return {
    ...
    status_counts: buildNormalizedPromiseStatusCounts(statusCountRows),
    pagination: {
      page,
      page_size: pageSize,
      total,
...
export async function fetchPromisePresidentDetail(slug, { showAll = false } = {}) {
  ...
  const visiblePromiseRows =
    slug === CURRENT_ADMINISTRATION_PROMISE_SLUG
      ? promiseRows.filter(isPublicCurrentAdminPromiseRow)
      : promiseRows;
  const visibleDeliveredCount = visiblePromiseRows.filter(
    (row) => row.status === "Delivered"
  ).length;
  const visibleInProgressCount = visiblePromiseRows.filter(
    (row) => row.status === "In Progress"
  ).length;
  const visiblePartialCount = visiblePromiseRows.filter(
    (row) => row.status === "Partial"
  ).length;
  const visibleFailedCount = visiblePromiseRows.filter(
    (row) => row.status === "Failed"
  ).length;
  const visibleBlockedCount = visiblePromiseRows.filter(
    (row) => row.status === "Blocked"
  ).length;

  return {
    ...president,
    total_tracked_promises:
      slug === CURRENT_ADMINISTRATION_PROMISE_SLUG
        ? visiblePromiseRows.length
        : president.total_tracked_promises,
    delivered_count:
      slug === CURRENT_ADMINISTRATION_PROMISE_SLUG
        ? visibleDeliveredCount
        : president.delivered_count,
    in_progress_count:
      slug === CURRENT_ADMINISTRATION_PROMISE_SLUG
        ? visibleInProgressCount
        : president.in_progress_count,
    partial_count:
      slug === CURRENT_ADMINISTRATION_PROMISE_SLUG
        ? visiblePartialCount
        : president.partial_count,
    failed_count:
      slug === CURRENT_ADMINISTRATION_PROMISE_SLUG
        ? visibleFailedCount
        : president.failed_count,
    blocked_count:
      slug === CURRENT_ADMINISTRATION_PROMISE_SLUG
        ? visibleBlockedCount
        : president.blocked_count,
    visible_promise_count: visiblePromiseRows.length,
    visible_outcome_count: visiblePromiseRows.reduce((count, row) => count + Number(row.outcome_count || 0), 0),
    visible_source_count: visiblePromiseRows.reduce((count, row) => count + Number(row.source_count || 0), 0),
    visible_delivered_count: visibleDeliveredCount,
    visible_in_progress_count: visibleInProgressCount,
    visible_partial_count: visiblePartialCount,
    visible_failed_count: visibleFailedCount,
    visible_blocked_count: visibleBlockedCount,
    show_all: showAll,
    status_sections: groupPromisesByStatus(visiblePromiseRows),
  };
}
...
export async function fetchCurrentAdministrationOverview(
  ...
  const visiblePromiseRows = promiseRows.filter(isPublicCurrentAdminPromiseRow);
  const publicGoverningActions = await fetchPublicGoverningActionsForPresident(president.id);
  const visiblePromiseIds = visiblePromiseRows.map((row) => row.id);

  const impactRows = visiblePromiseIds.length
    ? await query(
        `
        SELECT
          ...
          COUNT(*) AS total
        FROM promise_outcomes pout
        WHERE pout.promise_id IN (${visiblePromiseIds.map(() => "?").join(", ")})
  ...
  const recentActivityRows = visiblePromiseIds.length
    ? await query(
        `
        SELECT
          p.id,
        ...
        FROM promises p
        WHERE p.id IN (${visiblePromiseIds.map(() => "?").join(", ")})
        ORDER BY COALESCE(
          (
            SELECT MAX(pact_latest.action_date)
            FROM promise_actions pact_latest
  ...
  const topicBuckets = visiblePromiseRows.reduce((bucketMap, promise) => {
  ...
  for (const slug of featuredSlugs) {
    const record = visiblePromiseRows.find((promise) => promise.slug === slug);
    if (record) {
  ...
  for (const promise of visiblePromiseRows) {
  ...
  const promiseRowBySlug = new Map(visiblePromiseRows.map((row) => [row.slug, row]));
  ...
  return {
    ...
    president,
    total_promises: visiblePromiseRows.length,
    total_tracked_promises: visiblePromiseRows.length,
    total_promise_only: visiblePromiseRows.filter(
      (row) =>
        Number(row.public_governing_action_count || 0) === 0 &&
        Number(row.outcome_count || 0) === 0
    ).length,
    total_actions: visiblePromiseRows.reduce((count, row) => count + Number(row.action_count || 0), 0),
    total_governing_actions: publicGoverningActions.length,
    total_scoreable_governing_actions: countPublicScoreableGoverningActions(publicGoverningActions),
    total_scored_governing_actions: publicGoverningActions.filter((row) =>
      normalizeString(row.visibility_state) === "public_scored" || normalizeString(row.score_eligibility) === "scored"
    ).length,
    total_outcomes: visiblePromiseRows.reduce((count, row) => count + Number(row.outcome_count || 0), 0),
    impact_breakdown: impactBreakdown,
    tracked_promises: visiblePromiseRows,
    promise_only_records: visiblePromiseRows.filter(
      (row) =>
        Number(row.public_governing_action_count || 0) === 0 &&
        Number(row.outcome_count || 0) === 0
    ),
    governing_actions: publicGoverningActions,
    recent_governing_actions: publicGoverningActions.slice(0, 8),
...
export async function fetchPromiseDetail(slug) {
  ...
  const governingActionsMap = await fetchGoverningActionRowsForPromiseIds([promise.id], { publicOnly: true });
  const promiseSourceVisibilityRows = await query(
    `
    SELECT
      COUNT(*) AS promise_public_source_count
    FROM promise_sources ps
    WHERE ps.promise_id = ?
      AND ps.evidence_role = 'promise_attribution'
      AND ps.source_quality IN ('primary_official', 'secondary_verified')
  ...
  const promisePublicSourceCount = Number(
    promiseSourceVisibilityRows[0]?.promise_public_source_count || 0
  );
  ...
  const outcomeRows = await query(
    ...
    SELECT
      pout.id,
      pout.promise_id,
      pout.outcome_summary,
      pout.outcome_type,
      pout.measurable_impact,
      pout.impact_direction,
      pout.black_community_impact_note,
      pout.evidence_strength,
      pout.status_override,
      po.id AS policy_outcome_id,
      po.policy_type,
      po.source_count AS unified_source_count,
      po.source_quality,
      po.impact_score,
      po.confidence_score,
    ...
    FROM promise_outcomes pout
    LEFT JOIN policy_outcomes po
      ON po.policy_type = 'current_admin'
      AND po.policy_id = pout.promise_id
      AND po.outcome_summary_hash = SHA2(TRIM(pout.outcome_summary), 256)
    WHERE pout.promise_id = ?
    ORDER BY pout.id ASC
    `,
    [promise.id]
  );
  ...
  const governing_actions = (governingActionsMap.get(promise.id) || []).map((row) => ({
    ...row,
    action_sources: row.sources || [],
  }));
  ...
  const [relationshipData, demographicImpacts] = await Promise.all([
    getPromiseRelationships(promise.id),
    fetchEntityDemographicImpacts("promise", promise.id, { includeSources: true }),
  ]);

  const derivedVisibilityState =
    promise.president_slug === CURRENT_ADMINISTRATION_PROMISE_SLUG
      ? deriveCurrentAdminPromiseVisibilityState({
          public_governing_action_count: governing_actions.length,
          public_scored_governing_action_count: governing_actions.filter(
            (row) =>
              normalizeString(row.visibility_state) === "public_scored" ||
              normalizeString(row.score_eligibility) === "scored"
          ).length,
          outcome_count: outcomes.length,
          promise_public_source_count: promisePublicSourceCount,
        })
      : null;

  if (
    promise.president_slug === CURRENT_ADMINISTRATION_PROMISE_SLUG &&
    derivedVisibilityState === "internal_only"
  ) {
    return null;
  }

  const blackImpactSummary = buildPromiseBlackImpactSummary(promise, outcomes);
  const currentAdminRolloutParity =
    promise.president_slug === CURRENT_ADMINISTRATION_PROMISE_SLUG
      ? buildCurrentAdminRolloutParity({
          promise,
          governingActions: governing_actions,
          legacyActionCount: actions.length,
          legacyOutcomeCount: outcomes.length,
          legacyScoreSummary: blackImpactSummary,
        })
      : null;
  ...
  return applyPromiseCuration({
    ...
    governing_actions,
    outcomes,
    tracked_but_not_scored: governing_actions.length === 0 && outcomes.length === 0,
    visibility_state:
      derivedVisibilityState ||
      (governing_actions.some((row) => row.visibility_state === "public_scored") || outcomes.length > 0
        ? "public_scored"
        : "public_unscored"),
    score_eligibility: governing_actions.some((row) => row.score_eligibility === "scored")
      ? "scored"
      : outcomes.length > 0
        ? "scored"
        : governing_actions.some((row) => row.score_eligibility === "eligible")
        ? "eligible"
        : "not_scoreable",
    black_impact_summary: blackImpactSummary,
    current_admin_rollout_parity: currentAdminRolloutParity,
...
export async function fetchCurrentAdministrationRolloutParityReport(
  presidentSlug = CURRENT_ADMINISTRATION_PROMISE_SLUG
) {
  const overview = await fetchCurrentAdministrationOverview(presidentSlug);
  ...
  const detailRows = await Promise.all(
    (overview.tracked_promises || []).map((item) => fetchPromiseDetail(item.slug))
  );

  const rows = detailRows
    .filter(Boolean)
    .map((promise) => ({
      promise_slug: promise.slug,
      promise_title: promise.title,
      status: promise.status || null,
      legacy_promise_level_score:
        promise.current_admin_rollout_parity?.legacy_promise_level_score || {
          state: "unscored",
          total_score: null,
          direction: null,
      ...
      linked_governing_action_score_state:
        promise.current_admin_rollout_parity?.linked_governing_action_score_state || {
          state: "no_public_governing_action",
          public_action_count: Number(promise.governing_actions?.length || 0),
          scoreable_action_count: 0,
          scored_action_count: 0,
          visibility_states: [],
        },
      source_evidence_gap_reason:
        promise.current_admin_rollout_parity?.source_evidence_gap_reason || null,
      gap_reason_code: promise.current_admin_rollout_parity?.gap_reason_code || null,
      recommended_next_action:
        promise.current_admin_rollout_parity?.recommended_next_action ||
        "no_rollout_action_needed",
      has_mismatch: Boolean(promise.current_admin_rollout_parity?.has_mismatch),
    }));

  return {
    president_slug: presidentSlug,
    generated_at: new Date().toISOString(),
    total_promises: rows.length,
    mismatch_count: rows.filter((row) => row.has_mismatch).length,
    rows,
    mismatches: rows.filter((row) => row.has_mismatch),
  };
}
