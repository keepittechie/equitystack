...
export async function fetchPromisePresidentIndex() {
  const rows = await query(
    `
    SELECT
      pr.id,
...
export async function fetchPromisePresidentDetail(slug) {
  const presidentRows = await fetchPromisePresidentIndex();
