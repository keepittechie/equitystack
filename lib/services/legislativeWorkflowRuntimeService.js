...
export async function saveLegislativeApprovals({ bundlePath, actionUpdates }) {
  ...
  return {
    bundlePath: path.basename(REVIEW_BUNDLE_PATH),
...
export async function runLegislativeRunWithOptions({
  reviewModel = "",
  verifierModel = "",
  fallbackModel = "",
  timeout = "",
  ...
  if (applySafeRemovals) {
    args.push("--apply-safe-removals");
  }
  if (csv) {
    args.push("--csv");
  }
  if (normalizeString(outputReport)) {
    args.push("--output-report", normalizeString(outputReport));
  }
  for (const futureBillId of onlyFutureBillIds) {
    if (normalizeString(futureBillId)) {
      args.push("--only-future-bill-id", normalizeString(futureBillId));
    }
  }
