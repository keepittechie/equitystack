...
import { SOURCE_TYPES } from "@/lib/admin/promiseValidation.js";
...
const VALID_SOURCE_TYPES = new Set(SOURCE_TYPES);
...
function inferSourceTypeFromUrl(value) {
  ...
  if (
    domain === "archives.gov" ||
    domain.endsWith(".archives.gov") ||
    domain === "loc.gov" ||
    domain.endsWith(".loc.gov")
  ) {
    return "Archive";
  }
...
function normalizeSourceType(value, sourceUrl) {
  const text = normalizeString(value);
  if (VALID_SOURCE_TYPES.has(text)) {
    return text;
  }
...
function normalizeDraftPublishedDate(value) {
  const text = normalizeString(value);
...
function normalizeDraftRecommendation(row) {
  if (!row) {
    return null;
  }

  const sourceUrl = normalizeString(row.source_url || row.url);
  const sourceDate = normalizeString(row.source_date_raw || row.source_date);
  const publishedDate = normalizeDraftPublishedDate(sourceDate);

  return {
    id: Number(row.id || 0),
    import_file: normalizeString(row.import_file),
    import_row_number: Number(row.import_row_number || 0),
    record_type: normalizeString(row.record_type || row.row_type),
    record_id: Number(row.record_id || row.row_id || 0),
    president_name: normalizeString(row.president_name || row.president),
    bucket_name: normalizeString(row.bucket_name || row.bucket),
    source_title: normalizeString(row.source_title),
    source_url: sourceUrl,
    source_date: sourceDate || null,
    published_date: publishedDate,
    source_type: normalizeSourceType(row.source_type, sourceUrl),
    fit: normalizeString(row.fit) || "unknown",
    recommended_use: normalizeString(row.recommended_use) || "unspecified",
    notes: normalizeString(row.notes) || null,
    imported_at: toIsoOrNull(row.imported_at),
  };
}

function buildSuggestedNewSourceDraft(recommendation) {
  if (!recommendation) {
    return null;
  }
...
async function fetchMissingSourceRowsByIds(recordType, recordIds = []) {
  const ids = uniqueStrings(recordIds.map((value) => String(value))).map((value) => Number(value));
  if (!ids.length) {
    return [];
  }
...
async function fetchDirectRecordSourceMap(items = []) {
  const actionIds = uniqueStrings(
    items
      .filter((item) => item.record_type === "action")
  ...
  const outcomeIds = uniqueStrings(
    items
      .filter((item) => item.record_type === "outcome")
      .map((item) => String(item.record_id))
  )
    .map((value) => Number(value))
    .filter(Boolean);
  ...
  if (actionIds.length) {
    ...
    const [rows] = await db.execute(
      `
      SELECT
        pas.promise_action_id AS record_id,
        s.id,
        s.policy_id,
        s.source_title,
        s.source_url,
    ...
    for (const row of rows) {
      const reviewKey = buildMissingSourceReviewKey("action", row.record_id);
      if (!directSourceMap.has(reviewKey)) {
        directSourceMap.set(reviewKey, []);
      }
      directSourceMap.get(reviewKey).push(normalizeSourceRow(row));
    }
  }
  ...
  if (outcomeIds.length) {
    ...
    const [rows] = await db.execute(
      ...
      ORDER BY pout.id ASC, s.id ASC
      `,
      outcomeIds
    );

    for (const row of rows) {
      const reviewKey = buildMissingSourceReviewKey("outcome", row.record_id);
      if (!directSourceMap.has(reviewKey)) {
        directSourceMap.set(reviewKey, []);
      }
      directSourceMap.get(reviewKey).push(normalizeSourceRow(row));
...
async function fetchImportedDraftRecommendations(items = []) {
  const actionIds = uniqueStrings(
    items
      .filter((item) => item.record_type === "action")
      .map((item) => String(item.record_id))
  ...
  try {
    if (actionIds.length) {
      ...
      for (const row of rows) {
        const normalized = normalizeDraftRecommendation(row);
        if (!normalized) {
          continue;
        }
        const reviewKey = buildMissingSourceReviewKey("action", normalized.record_id);
        if (!recommendationMap.has(reviewKey)) {
          recommendationMap.set(reviewKey, []);
        }
        recommendationMap.get(reviewKey).push(normalized);
      }
    }
    ...
    if (outcomeIds.length) {
      ...
      for (const row of rows) {
        ...
        const reviewKey = buildMissingSourceReviewKey("outcome", normalized.record_id);
        if (!recommendationMap.has(reviewKey)) {
          recommendationMap.set(reviewKey, []);
        }
        recommendationMap.get(reviewKey).push(normalized);
...
async function resolvePolicyOutcomeIdForRecord(recordType, recordId, connection = null) {
...
async function syncCanonicalOutcomeSourceMetadata(policyOutcomeId, connection) {
  if (!policyOutcomeId) {
    return;
  }
  const [rows] = await connection.execute(
    ...
    FROM policy_outcome_sources pos
    JOIN sources s ON s.id = pos.source_id
    WHERE pos.policy_outcome_id = ?
    `,
    [policyOutcomeId]
  );
  const sourceIds = new Set();
  let hasHighAuthority = false;
  let hasMediumAuthority = false;
  for (const row of rows || []) {
    const sourceId = Number(row.id || 0);
    if (sourceId) {
      sourceIds.add(sourceId);
    }
    const normalizedType = normalizeString(row.source_type).toLowerCase();
    const normalizedPublisher = normalizeString(row.publisher).toLowerCase();
    const domain = normalizeSourceDomain(row.source_url);
    if (
      ...
      domain === "govinfo.gov" ||
      domain === "whitehouse.gov" ||
      normalizedPublisher.includes("congress") ||
      normalizedPublisher.includes("white house")
    ) {
      hasHighAuthority = true;
    } else if (domain.endsWith(".edu") || normalizedType === "news" || normalizedType === "nonprofit") {
      hasMediumAuthority = true;
    }
  }
  const sourceCount = sourceIds.size;
  const sourceQuality =
    sourceCount < 1 ? null : hasHighAuthority ? "high" : hasMediumAuthority ? "medium" : "low";
  await connection.execute(
    `
    UPDATE policy_outcomes
    SET source_count = ?, source_quality = ?
    WHERE id = ?
    `,
    [sourceCount, sourceQuality, policyOutcomeId]
  );
}
  ...
  if (recordType === "action") {
    [rows] = await executor.execute(
      ...
      SELECT
        ...
        s.publisher,
        s.published_date
      FROM promise_action_sources j
      JOIN sources s ON s.id = j.source_id
      WHERE j.promise_action_id = ?
      ORDER BY s.id ASC
      `,
      [recordId]
    );
  } else {
    const policyOutcomeId = await resolvePolicyOutcomeIdForRecord(recordType, recordId, executor);
    if (!policyOutcomeId) {
      rows = [];
    } else {
      [rows] = await executor.execute(
        ...
        SELECT
          ...
          s.publisher,
          s.published_date
        FROM policy_outcome_sources j
        JOIN sources s ON s.id = j.source_id
        WHERE j.policy_outcome_id = ?
        ORDER BY s.id ASC
        `,
        [policyOutcomeId]
      );
...
async function insertSourceJoin(recordType, recordId, sourceId, connection) {
  ...
  const policyOutcomeId = await resolvePolicyOutcomeIdForRecord(recordType, recordId, connection);
  if (!policyOutcomeId) {
    throw new Error("The selected outcome no longer has a matching policy_outcomes row.");
  }
  await connection.execute(
    `
    ...
    VALUES (?, ?)
    `,
    [policyOutcomeId, sourceId]
  );
  await syncCanonicalOutcomeSourceMetadata(policyOutcomeId, connection);
}
...
async function createSourceRow(draft, connection) {
  const [result] = await connection.execute(
    ...
    [
      ...
      normalizeSourceType(draft.source_type, draft.source_url),
...
async function buildCurrentMissingSourceItems(artifactRows = []) {
  ...
  const [{ promiseContextMap, policyContextMap }, directSourceMap, importedDraftMap] = await Promise.all([
    fetchPromiseContextMap(mergedItems),
    fetchDirectRecordSourceMap(mergedItems),
    fetchImportedDraftRecommendations(mergedItems),
  ]);
  return mergedItems.map((item) => {
    ...
    const directRecordSources = directSourceMap.get(item.review_key) || [];
    const draftRecommendations = importedDraftMap.get(item.review_key) || [];
    const preferredDraftRecommendation = draftRecommendations[0] || null;
    return {
      ...item,
      direct_record_sources: directRecordSources,
      direct_record_source_count: directRecordSources.length,
      ...
      draft_recommendation_count: draftRecommendations.length,
      suggested_new_source_draft: buildSuggestedNewSourceDraft(preferredDraftRecommendation),
...
async function buildRetainedSavedMissingSourceItems(currentItems = [], decisions = []) {
  const currentReviewKeys = new Set(currentItems.map((item) => item.review_key));
  const retainedDecisions = decisions.filter((decision) => !currentReviewKeys.has(decision.reviewKey));
  if (!retainedDecisions.length) {
    return [];
  }

  const actionIds = retainedDecisions
    .filter((decision) => decision.recordType === "action")
    .map((decision) => decision.recordId);
  const outcomeIds = retainedDecisions
    .filter((decision) => decision.recordType === "outcome")
    .map((decision) => decision.recordId);

  const [actionItems, outcomeItems] = await Promise.all([
    fetchMissingSourceRowsByIds("action", actionIds),
    fetchMissingSourceRowsByIds("outcome", outcomeIds),
  ]);

  const dbItems = [...actionItems, ...outcomeItems];
  const dbItemMap = new Map(dbItems.map((item) => [item.review_key, item]));

  const retainedItems = retainedDecisions.map((decision) => {
    const dbItem = dbItemMap.get(decision.reviewKey);
    if (dbItem) {
      return {
        ...dbItem,
        import_origin: decision.importOrigin || dbItem.import_origin,
        retained_after_saved_decision: true,
      };
    }

    return normalizeMissingSourceItem({
      review_key: decision.reviewKey,
      record_type: decision.recordType,
      record_id: decision.recordId,
      promise_id: decision.promiseId,
      promise_slug: decision.promiseSlug,
      promise_title: decision.promiseTitle,
      president_slug: decision.presidentSlug,
      president_name: decision.presidentName,
      topic: decision.topic,
      likely_import_origin: decision.importOrigin,
      record_text: "",
      record_detail: "",
      retained_after_saved_decision: true,
    });
  ...
  const [{ promiseContextMap, policyContextMap }, directSourceMap, importedDraftMap] = await Promise.all([
    fetchPromiseContextMap(retainedItems),
    fetchDirectRecordSourceMap(retainedItems),
    fetchImportedDraftRecommendations(retainedItems),
  ]);
  ...
  return retainedItems.map((item) => {
    ...
    return {
      ...
      suggested_new_source_draft: buildSuggestedNewSourceDraft(preferredDraftRecommendation),
      unresolved_reason:
        "This row already has a saved source-curation decision and stays visible until the group review is explicitly completed.",
      suggested_next_step:
        "Review the saved decision, then attach another source or update the note if needed.",
      retained_after_saved_decision: true,
    };
  });
}
...
export async function getSourceCurationWorkspace() {
  ...
  let retainedSavedItems = [];
  try {
    retainedSavedItems = await buildRetainedSavedMissingSourceItems(
      missingSourceItems,
      store.missingSourceDecisions
    );
  } catch (error) {
    missingSourceLoadError =
      missingSourceLoadError ||
      (error instanceof Error
        ? error.message
        : "Failed to retain previously reviewed source-curation rows from the DB.");
  }

  const enrichedMissingItems = mergeDecisionMaps(
    [...missingSourceItems, ...retainedSavedItems],
    missingDecisionMap,
    buildStoredMissingDecisionSummary
  );
