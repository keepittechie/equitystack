...
function buildPromiseInsight(items = []) {
  ...
  if (delivered || blocked || failed) {
    return {
      ...
      text: `The current promise snapshot shows ${formatCount(delivered)} Delivered promises, ${formatCount(blocked)} Blocked promises, and ${formatCount(failed)} Failed promises.`,
    };
  }
  ...
  return {
    title: "Promise Tracker",
    text: `${leader.name} is the largest promise-status group in the current snapshot (${formatCount(leader.value)} records).`,
  };
}

function buildPresidentRankingInsight(presidents = []) {
  const ranked = presidents
    .filter((item) => Number.isFinite(Number(item.direct_normalized_score)))
    .slice()
    .sort(
      (left, right) =>
  ...
  return {
    ...
    text: `Current presidential direct Impact Scores range from ${topName} (${formatScoreValue(top.direct_normalized_score)}) to ${bottomName} (${formatScoreValue(bottom.direct_normalized_score)}).`,
  };
}

function buildReportInsights({ scores, categorySummary, promiseItems = [], presidents = [] }) {
  return [
    buildDirectionInsight(scores?.records || []),
    buildCategoryInsight(categorySummary || []),
    buildPromiseInsight(promiseItems || []),
...
export async function fetchHomePageData() {
  const [scores, readiness, policies, promises, presidents, overallSummary, categorySummary] = await Promise.all([
  ...
  return {
    ...
    findings: [
      ...
      hub.scores.metadata?.summary_interpretation
        ...
        : null,
      buildDirectionInsight(hub.scores.records || []),
      buildPromiseInsight(dashboard.promiseSnapshot.items || []),
    ].filter(Boolean),
    topPolicies: {
