...
function buildGeneratedArgumentMode(explainer = {}, sections = [], argumentReady = null) {
  ...
  const claim =
    truncateSentence(
      argumentReady?.claim ||
        ...
        `The headline claim about ${explainer.title || "this topic"} proves more than the record actually supports.`
    ) || "The headline claim proves more than the record actually supports.";
  ...
  const keyPoints = uniqueTextList([
    ...
    supportPoint
      ? `Evidence type: ${supportPoint}`
      : "Evidence type matters: distinguish primary records, implementation records, and interpretation before drawing a conclusion.",
    limitationPoint
      ? `Limit: ${limitationPoint}`
      : "The record should separate existence of a case, scale across the measured population, and impact on outcomes.",
    contextPoint,
    ...keyTakeaways,
    "A stronger claim would need evidence that matches the claim's scale, not only examples or broad labels.",
  ]).slice(0, 7);
  ...
  const commonClaims = argumentReady
    ? [
        {
          ...
          question:
            "Which source or record type proves the full claim, rather than only a narrower point?",
        },
        {
          claim: "This evidence proves the strongest possible version of the claim.",
          response: supportsButNotProvesResponse,
          question:
            "What evidence turns this from a bounded claim into the strongest version being asserted?",
        },
        {
          claim: "If the evidence is mixed or not fully quantified, the concern is unsupported.",
          response: mixedEvidenceResponse,
          question:
            "What part of the claim is already supported, and what part still needs stronger outcome or causal evidence?",
        },
        {
          claim: "One example settles the whole issue.",
          response:
    ...
    : [
        ...
        {
          claim: "The observed disparity proves a single cause.",
          response:
  ...
  const debateLines = argumentReady
    ? uniqueTextList([
        strongestResponse,
        "Use the narrowest claim the records clearly support.",
        "Ask what the evidence measures directly, not what people want it to imply.",
        "Separate baseline evidence, implementation evidence, and outcome evidence.",
      ]).slice(0, 5)
    : [
        "Separate the existence of evidence from its scale and its impact.",
        "A claim about broad impact needs evidence measured at broad scale.",
        "The strongest argument is the one the records can actually support.",
      ];
  const shareCards = argumentReady
    ? [
        {
          title: "Start with the bounded answer",
          text:
            argumentReady.bottomLine ||
        ...
        {
          title: "What stronger proof would need",
          text:
