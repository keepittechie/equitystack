...
import {
  getPresidentHeadlineScoreValue,
  sortPresidentsByHeadlineScore,
} from "@/lib/black-impact-score/presidentHeadlineScore";
import { getExplainerEditorial } from "@/lib/explainer-editorial";
import { getPolicyVisualEvidence } from "@/lib/policy-visual-evidence";
import { getPromiseVisualEvidence } from "@/lib/promise-visual-evidence";
...
const ALL_VISUAL_TYPES = new Set([
  "map",
  "chart",
  "source-snapshot",
  "system-breakdown",
...
const POLICY_DETAIL_VISUAL_TYPES = new Set(["map", "chart", "source-snapshot"]);
const PROMISE_DETAIL_VISUAL_TYPES = new Set(["chart", "source-snapshot"]);
const ALLOWED_VISUAL_SOURCE_TYPES = new Set([
  "government",
  "academic",
  "primary-data",
  "legal",
...
function normalizeVisualEvidence(items = [], allowedTypes = ALL_VISUAL_TYPES) {
  return normalizeStructuredList(items)
    .map((item) => {
      ...
      return {
        ...
        sourceType: ALLOWED_VISUAL_SOURCE_TYPES.has(sourceType) ? sourceType : "",
...
function normalizePlacementBindings(value) {
...
function normalizeExplainerVisualEvidence(items = []) {
  return normalizeVisualEvidence(items, ALL_VISUAL_TYPES);
}
...
function buildPresidentRankingInsight(presidents = []) {
  const ranked = presidents
    .filter((item) => Number.isFinite(getPresidentHeadlineScoreValue(item)))
    .slice();
  const sorted = sortPresidentsByHeadlineScore(ranked);
  ...
  return {
    ...
    text: `Current presidential Black Impact Scores range from ${topName} (${formatScoreValue(getPresidentHeadlineScoreValue(top))}) to ${bottomName} (${formatScoreValue(getPresidentHeadlineScoreValue(bottom))}).`,
  };
}
...
function buildPresidentRecord(item = {}, promiseMeta = {}, billInputs = {}) {
  const headlineScore = getPresidentHeadlineScoreValue(item);
  ...
  return {
    ...
    score: headlineScore,
    overall_ranking_score: item.overall_ranking_score ?? item.ranking_score_total ?? headlineScore,
    ranking_score_total: item.ranking_score_total ?? headlineScore,
    normalized_score_total: item.normalized_score_total ?? item.direct_normalized_score,
    raw_average_score: item.raw_average_score ?? item.normalized_score_total ?? item.direct_normalized_score,
    display_score: item.display_score_total ?? item.display_score ?? null,
    coverage_adjusted_score:
      item.coverage_adjusted_score ?? item.display_score_total ?? item.display_score ?? null,
    ...
    evidence_confidence: item.evidence_confidence || item.direct_score_confidence || item.score_confidence,
    ...
    tone: getDirectionTone(headlineScore ?? item.normalized_score_total ?? item.direct_raw_score),
...
export async function fetchDashboardData(searchParams = {}) {
  ...
  return {
    ...
    presidentRanking: presidentContext.presidents
      .slice()
      .sort((left, right) => getPresidentHeadlineScoreValue(right) - getPresidentHeadlineScoreValue(left))
      .slice(0, 6),
...
export async function fetchPresidentsOverviewData(searchParams = {}) {
  ...
  presidents = presidents.slice().sort((left, right) => {
    ...
    return getPresidentHeadlineScoreValue(right) - getPresidentHeadlineScoreValue(left);
  });
...
export async function fetchPolicyDetailBySlug(slug) {
  ...
  const policyVisualEvidence = getPolicyVisualEvidence(policy);

  return {
    ...policy,
    slug: buildPolicySlug(policy),
    suggested_explainers: suggestedExplainers,
    visualEvidence: normalizeVisualEvidence(
      policyVisualEvidence.visualEvidence,
      POLICY_DETAIL_VISUAL_TYPES
    ),
    visualEvidenceBindings: normalizePlacementBindings(
      policyVisualEvidence.visualEvidenceBindings
    ),
  };
}
...
export async function fetchPromisePageData(slug) {
  ...
  const reviewSummary = [
    ...
    .filter(Boolean)
    .join(" ");
  const promiseVisualEvidence = getPromiseVisualEvidence(promise);
  ...
  return {
    ...
    visualEvidence: normalizeVisualEvidence(
      promiseVisualEvidence.visualEvidence,
      PROMISE_DETAIL_VISUAL_TYPES
    ),
    visualEvidenceBindings: normalizePlacementBindings(
      promiseVisualEvidence.visualEvidenceBindings
    ),
  };
}
...
export async function fetchExplainerDetailData(slug, options = {}) {
  ...
  return {
    ...
    editorial_category: normalizeText(editorial.category),
    editorial_category_label: formatSlugLabel(editorial.category),
    visualEvidence: normalizeExplainerVisualEvidence(editorial.visualEvidence),
