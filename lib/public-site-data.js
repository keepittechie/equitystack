...
function buildPromiseInsight(items = []) {
  ...
  const constrained = Number(counts.Blocked || 0) + Number(counts.Failed || 0);

  if (delivered || constrained) {
    return {
      title: "Promise Tracker",
      text: `The current promise snapshot shows ${formatCount(delivered)} delivered promises and ${formatCount(constrained)} blocked or failed promises.`,
    };
  }
...
export async function fetchDashboardData(searchParams = {}) {
  ...
  return {
    ...
    insights: buildReportInsights({
      scores,
      categorySummary,
      promiseItems: recentPromises.items || [],
