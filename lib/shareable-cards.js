...
import {
  ...
  buildPolicyCardHref,
  buildPresidentCardHref,
  buildPromiseCardHref,
  parseTrailingId,
} from "@/lib/shareable-card-links";
import { fetchPresidentProfileData } from "@/lib/public-site-data";
...
function buildFutureBillSources(bill) {
      ...
      if (trackedBill.url) {
        items.push(
          normalizeSource({
            ...
            notes: trackedBill.link_notes || null,
          })
        );
      }
...
export async function getFutureBills() {
...
export async function getFutureBillDetail(slug) {
  ...
  const trackedBills = Array.isArray(bill.tracked_bills) ? bill.tracked_bills : [];
  const sources = buildFutureBillSources(bill);

  return {
    ...bill,
    slug,
    detailPath: buildFutureBillDetailHref(bill),
    cardPath: buildFutureBillCardHref(bill),
...
export async function getPresidentCard(slug) {
  ...
  const title = president.president || president.president_name || "President";
  const topicDrivers = (scoreDrivers?.topic_drivers || [])
    .map((item) => item.topic)
    .filter(Boolean)
    .slice(0, 3);
  const summary =
    ...
    `EquityStack profile for ${title}.`;

  return {
    kind: "president",
    title,
    category: president.president_party || "President",
    summary,
    sharePath: buildPresidentCardHref(president),
    canonicalPath: buildPresidentCardHref(president),
    impactContext:
      scoreComposition?.interpretation ||
      "This card summarizes the current presidential record, score context, and visible driver mix.",
    keyData: [
      { label: "Term", value: president.termLabel || "Historical record" },
      {
        label: "Black Impact Score",
        value:
          president.overall_ranking_score != null
            ? Number(president.overall_ranking_score).toFixed(2)
            : president.score != null
            ? Number(president.score).toFixed(2)
            : president.normalized_score_total != null
              ? Number(president.normalized_score_total).toFixed(2)
            : "N/A",
      },
      {
        label: "Confidence",
      ...
      {
        label: "Outcomes",
        value: String(president.direct_outcome_count ?? president.outcome_count ?? 0),
      },
      {
        label: "Promises",
        value: String(
    ...
    sections: [
      {
        title: "What Shaped This Score",
        body:
          scoreComposition?.summary_line ||
          "This presidential score is derived from linked outcome evidence and supporting public record context.",
      },
    ],
    linkedRecords: [
      ...(scoreDrivers?.strongest_positive || []).slice(0, 2).map((item) => ({
        title: item.title,
        detail: ["Positive driver", item.topic || item.category || null]
          .filter(Boolean)
          .join(" • "),
      })),
      ...(scoreDrivers?.strongest_negative || []).slice(0, 2).map((item) => ({
        title: item.title,
        detail: ["Negative driver", item.topic || item.category || null]
          .filter(Boolean)
          .join(" • "),
      })),
      ...(topicDrivers.length
        ? [
            {
              ...
              detail: topicDrivers.join(" • "),
            },
          ]
        : []),
    ],
    sources: dedupeSources(
      (promiseTracker.sources || []).map(normalizeSource).filter(Boolean)
    ),
  };
}
