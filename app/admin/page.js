          ...
          <tbody>
            {rows.length ? (
              rows.map((job) => {
                ...
                const itemsProcessed = isAiStep && workflowOutcome ? workflowOutcome.itemsProcessed || 0 : "N/A";
                const manualReviewCount =
                  isAiStep && workflowOutcome ? workflowOutcome.manualReviewCount || 0 : "N/A";
                const detailHref =
                  ...
                  <tr key={item.id} className="align-top odd:bg-[var(--admin-surface)] even:bg-[var(--admin-surface-muted)] hover:bg-[var(--admin-surface-soft)]">
                    ...
                    <td className="border-b border-[var(--admin-line)] px-2 py-1">
                      {item.href ? (
                        ...
                        <span className="text-[11px] text-[var(--admin-text-muted)]">N/A</span>
                      )}
                    </td>
                  </tr>
                );
...
function renderPrimaryAction(actionConfig, fallbackHref, fallbackLabel = "Inspect", context = {}) {
  ...
  return <span className="text-[11px] text-[var(--admin-text-muted)]">N/A</span>;
}
              ...
              sessions.slice(0, 8).map((session) => {
                ...
                return (
                  <tr key={session.id} className="align-top odd:bg-[var(--admin-surface)] even:bg-[var(--admin-surface-muted)] hover:bg-[var(--admin-surface-soft)]">
                    ...
                    <td className="border-b border-[var(--admin-line)] px-2 py-1 font-mono text-[10px] text-[var(--admin-text)]">
                      {session.linkedSchedule?.title || "N/A"}
                    </td>
                    <td className="border-b border-[var(--admin-line)] px-2 py-1">
