...
const DECISION_LABELS = {
  ...
  publish_promise_only: "publish_promise_only",
  publish_action: "publish_action",
  hold_for_evidence: "hold_for_evidence",
  reject_slogan: "reject_slogan",
  merge_existing_action: "merge_existing_action",
  link_existing_promise: "link_existing_promise",
};
...
function itemRecordShape(item) {
  const finalRecord = item?.final_record || {};
  const firstAction = Array.isArray(finalRecord.actions) ? finalRecord.actions[0] || {} : {};
  const actionSources = Array.isArray(firstAction?.sources) ? firstAction.sources : [];
  const evidenceRoles = [...new Set(
    actionSources
      .map((source) => normalizeString(source?.evidence_role))
      .filter(Boolean)
  )];
  return {
    entityDisposition: normalizeString(item?.entity_disposition || finalRecord.entity_disposition),
    mechanismType: normalizeString(item?.mechanism_type || firstAction.governing_action_type || firstAction.action_type),
    scoreEligibility: normalizeString(item?.score_eligibility || firstAction.score_eligibility || finalRecord.score_eligibility),
    visibilityState: normalizeString(item?.visibility_state || firstAction.visibility_state || finalRecord.visibility_state),
    governingActionSlug: normalizeString(firstAction?.slug),
    governingActionTitle: normalizeString(firstAction?.title),
    sourceQuality:
      normalizeString(firstAction?.best_source_quality) ||
      normalizeString(item?.source_quality) ||
      normalizeString(actionSources[0]?.source_quality),
    evidenceRoles,
    parentActionId: firstAction.parent_action_id,
    chainRelationshipType: normalizeString(firstAction.chain_relationship_type),
    rejectReason: normalizeString(item?.reject_reason_code || finalRecord.reject_reason_code),
    dedupeKey: normalizeString(item?.candidate_dedupe_key || firstAction.dedupe_key),
    likelyDuplicateActionId: item?.likely_duplicate_action_id || finalRecord.likely_duplicate_action_id || null,
  };
}

function summarizeEvidenceRoles(roles = []) {
  const values = Array.isArray(roles) ? roles.filter(Boolean) : [];
  if (!values.length) {
    return "None";
  }
  return values.map((role) => formatEvidenceRoleLabel(role)).join(", ");
}
...
export default function CurrentAdminReviewWorkspace({ workspace, rolloutReports = null }) {
  ...
  const parityRows = Array.isArray(rolloutReports?.parity_report?.rows)
    ? rolloutReports.parity_report.rows
    : [];
  const parityByPromiseSlug = new Map(
    parityRows
      .filter((row) => normalizeString(row?.promise_slug))
      .map((row) => [normalizeString(row.promise_slug), row])
  );
  const readinessActions = Array.isArray(rolloutReports?.readiness_report?.payload?.actions)
    ? rolloutReports.readiness_report.payload.actions
    : [];
  const readinessByPromiseSlug = new Map(
    readinessActions
      .filter((row) => normalizeString(row?.linked_promise_slug))
      .map((row) => [normalizeString(row.linked_promise_slug), row])
  );
  const readinessByActionSlug = new Map(
    readinessActions
      .filter((row) => normalizeString(row?.governing_action_slug))
      .map((row) => [normalizeString(row.governing_action_slug), row])
  );
  const scoringReviewActions = Array.isArray(rolloutReports?.scoring_review_report?.payload?.actions)
    ? rolloutReports.scoring_review_report.payload.actions
    : [];
  const scoringReviewByActionSlug = new Map(
    scoringReviewActions
      .filter((row) => normalizeString(row?.governing_action_slug))
      .map((row) => [normalizeString(row.governing_action_slug), row])
  );
  const eligibilityReviewBundles = Array.isArray(rolloutReports?.eligibility_review_report?.payload?.review_bundles)
    ? rolloutReports.eligibility_review_report.payload.review_bundles
    : [];
  const eligibilityByActionSlug = new Map(
    eligibilityReviewBundles
      .filter((row) => normalizeString(row?.governing_action_slug))
      .map((row) => [normalizeString(row.governing_action_slug), row])
  );
      ...
      {rolloutReports ? (
        <section className={SECTION_CLASS}>
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-[11px] uppercase tracking-wide text-[var(--admin-text-muted)]">Rollout parity</p>
              <h2 className="mt-1 text-base font-semibold text-[var(--admin-text)]">
                Governing-action rollout state stays visible during legacy coexistence
              </h2>
              <p className="mt-1 max-w-5xl text-[12px] text-[var(--admin-text-soft)]">
                Promise-level current-admin scores still remain visible during rollout. Governing actions, evidence
                gates, and readiness reports now tell operators exactly what is public, what is eligible, and what is
                still blocked before any new scoring pass.
              </p>
            </div>
            <div className={PANEL_CLASS}>
              <p className={LABEL_CLASS}>Latest parity / readiness artifacts</p>
              <p className="mt-1 text-[11px] text-[var(--admin-text-soft)]">
                Parity: {rolloutReports?.parity_report?.generated_at || "Unavailable"}
              </p>
              <p className="mt-1 text-[11px] text-[var(--admin-text-soft)]">
                Readiness: {rolloutReports?.readiness_report?.payload?.generated_at || "Unavailable"}
              </p>
              <p className="mt-1 text-[11px] text-[var(--admin-text-soft)]">
                Eligibility review: {rolloutReports?.eligibility_review_report?.payload?.generated_at || "Unavailable"}
              </p>
            </div>
          </div>
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <div className={CARD_CLASS}>
              <p className={LABEL_CLASS}>Parity mismatches</p>
              <p className="mt-1 text-base font-semibold text-[var(--admin-text)]">
                {rolloutReports?.parity_report?.mismatch_count || 0}
              </p>
              <p className="mt-1 text-[11px] text-[var(--admin-text-muted)]">
                Promise-level scores still visible while governing-action score state catches up.
              </p>
            </div>
            <div className={CARD_CLASS}>
              <p className={LABEL_CLASS}>Eligible public actions</p>
              <p className="mt-1 text-base font-semibold text-[var(--admin-text)]">
                {rolloutReports?.eligibility_review_report?.payload?.summary?.actions_marked_eligible || 0}
              </p>
              <p className="mt-1 text-[11px] text-[var(--admin-text-muted)]">
                Eligible means evidence-complete enough for a scoring pass, not already scored.
              </p>
            </div>
            <div className={CARD_CLASS}>
              <p className={LABEL_CLASS}>Scoring-ready actions</p>
              <p className="mt-1 text-base font-semibold text-[var(--admin-text)]">
                {rolloutReports?.readiness_report?.payload?.summary?.scoring_ready_count || 0}
              </p>
              <p className="mt-1 text-[11px] text-[var(--admin-text-muted)]">
                This stays separate from eligibility until the population-impact chain is fully supported.
              </p>
            </div>
            <div className={CARD_CLASS}>
              <p className={LABEL_CLASS}>Eligible but not ready</p>
              <p className="mt-1 text-base font-semibold text-[var(--admin-text)]">
                {Array.isArray(rolloutReports?.scoring_review_report?.payload?.eligible_but_not_ready_actions)
                  ? rolloutReports.scoring_review_report.payload.eligible_but_not_ready_actions.length
                  : 0}
              </p>
              <p className="mt-1 text-[11px] text-[var(--admin-text-muted)]">
                Use parity gaps and blocker reasons to decide whether more evidence or a mechanism review is needed.
              </p>
            </div>
          </div>
        </section>
            ...
            <tbody>
              {(items || []).map((item) => (
                <tr key={item.slug} className={TABLE_ROW_CLASS}>
                  <td className="border-b border-[var(--admin-line)] px-3 py-2">
                    {(() => {
                      const shape = itemRecordShape(item);
                      const parity = parityByPromiseSlug.get(normalizeString(item.slug));
                      const readiness =
                        readinessByPromiseSlug.get(normalizeString(item.slug)) ||
                        readinessByActionSlug.get(shape.governingActionSlug);
                      return (
                        <>
                    ...
                    <div className="mt-2 flex flex-wrap gap-1 text-[10px] text-[var(--admin-text-muted)]">
                      {shape.entityDisposition ? <span>{humanizeToken(shape.entityDisposition)}</span> : null}
                      {shape.mechanismType ? <span>• {humanizeToken(shape.mechanismType)}</span> : null}
                      {shape.scoreEligibility ? <span>• {humanizeToken(shape.scoreEligibility)}</span> : null}
                      {shape.visibilityState ? <span>• {humanizeToken(shape.visibilityState)}</span> : null}
                      {shape.sourceQuality ? <span>• {formatSourceQualityLabel(shape.sourceQuality)}</span> : null}
                    </div>
                    {parity?.gap_reason_code || readiness?.current_recommendation ? (
                      <div className="mt-2 text-[11px] text-[var(--admin-text-soft)]">
                        {parity?.gap_reason_code ? `Parity gap: ${humanizeToken(parity.gap_reason_code)}` : null}
                        {parity?.gap_reason_code && readiness?.current_recommendation ? " • " : null}
                        {readiness?.current_recommendation
                          ? `Readiness: ${humanizeToken(readiness.current_recommendation)}`
                          : null}
                      </div>
                    ) : null}
                        </>
                      ...
                      <div className="mt-2 space-y-2 text-[11px] text-[var(--admin-text-soft)]">
                        {(() => {
                          ...
                          const readiness =
                            readinessByPromiseSlug.get(normalizeString(item.slug)) ||
                            readinessByActionSlug.get(shape.governingActionSlug);
                          const scoringReview = scoringReviewByActionSlug.get(shape.governingActionSlug);
                          const eligibilityReview = eligibilityByActionSlug.get(shape.governingActionSlug);
                          const blockerDetails = formatBlockedDetails(item);
                          return (
                            <div>
                              <p className="font-medium text-[var(--admin-text)]">Record shape</p>
                              <div className="mt-1 space-y-1">
                                <p>Entity disposition: {shape.entityDisposition ? humanizeToken(shape.entityDisposition) : "Unknown"}</p>
                                <p>Mechanism type: {shape.mechanismType ? humanizeToken(shape.mechanismType) : "Unknown"}</p>
                                <p>Governing action: {shape.governingActionSlug || shape.governingActionTitle || "None yet"}</p>
                                <p>Score eligibility: {shape.scoreEligibility ? formatScoreEligibilityLabel(shape.scoreEligibility) : "Unknown"}</p>
                                <p>Visibility state: {shape.visibilityState ? formatVisibilityStateLabel(shape.visibilityState) : "Unknown"}</p>
                                <p>Source quality: {shape.sourceQuality ? formatSourceQualityLabel(shape.sourceQuality) : "Unknown"}</p>
                                <p>Evidence roles: {summarizeEvidenceRoles(shape.evidenceRoles)}</p>
                                <p>
                                  Scoring readiness:{" "}
                                  {typeof readiness?.scoring_ready === "boolean"
                                    ? readiness.scoring_ready
                                      ? "Ready"
                                      : "Not ready"
                                    : scoringReview
                                      ? scoringReview.scoring_ready
                                        ? "Ready"
                                        : "Not ready"
                                      : "Not reviewed"}
                                </p>
                                <p>
                                  ...
                                  {summarizeBlockers(
                                    readiness?.blockers ||
                                      scoringReview?.readiness_blockers ||
                                      blockerDetails
                                  )}
                                </p>
                                <p>Parent/child chain: {shape.parentActionId ? `${shape.parentActionId}${shape.chainRelationshipType ? ` (${humanizeToken(shape.chainRelationshipType)})` : ""}` : "None"}</p>
                                <p>Likely duplicate action: {shape.likelyDuplicateActionId || "None"}</p>
                                <p>Reject reason: {shape.rejectReason || "None"}</p>
                                <p>Dedupe key: {shape.dedupeKey || "Unavailable"}</p>
                                <p>Parity gap reason: {parity?.gap_reason_code ? humanizeToken(parity.gap_reason_code) : "None"}</p>
                                <p>Parity next action: {parity?.recommended_next_action ? humanizeToken(parity.recommended_next_action) : "None"}</p>
                                <p>
                                  Eligibility review:{" "}
                                  {eligibilityReview?.current_state?.public_ui_target_state || "Not in eligibility bundle"}
                                </p>
                              </div>
                            </div>
                          );
