...
import { getCurrentAdministrationRolloutReports } from "@/lib/services/currentAdministrationRolloutReportService";
...
export default async function CurrentAdminReviewPage() {
  const [workspace, rolloutReports] = await Promise.all([
    getCurrentAdministrationOperatorWorkspace(),
    getCurrentAdministrationRolloutReports(),
  ]);
  ...
  return (
    <main className="mx-auto max-w-[1700px] space-y-4 px-4 py-4">
      ...
      <CurrentAdminReviewWorkspace workspace={workspace} rolloutReports={rolloutReports} />
    </main>
  );
}
