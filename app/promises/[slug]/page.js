...
function countUniquePromiseEvidenceSources(promise) {
  ...
  for (const action of promise.governing_actions || []) {
    for (const source of action.action_sources || action.sources || []) {
      addSource(source);
    }
  }
...
function normalizeActionDedupKey(title, actionDate) {
  const normalizedTitle = normalizeInlineText(title).toLowerCase();
  const parsedDate = actionDate ? new Date(actionDate) : null;
  const normalizedDate =
    parsedDate && !Number.isNaN(parsedDate.getTime())
      ? parsedDate.toISOString().slice(0, 10)
      : normalizeInlineText(actionDate);
  return `${normalizedTitle}::${normalizedDate}`;
}

function buildVisibleLegacyPromiseActions(legacyActions = [], governingActions = []) {
  const governingActionKeys = new Set(
    (governingActions || []).map((action) =>
      normalizeActionDedupKey(action?.title, action?.action_date || action?.effective_date)
    )
  );

  return (legacyActions || []).filter((action) => {
    const key = normalizeActionDedupKey(action?.title, action?.action_date);
    return !governingActionKeys.has(key);
  });
}

function buildCombinedActionRecords(legacyActions = [], governingActions = []) {
  const visibleLegacyActions = buildVisibleLegacyPromiseActions(legacyActions, governingActions);
  const normalizedGoverningActions = (governingActions || []).map((action) => ({
    id: `governing-${action.id}`,
    action_date: action.action_date || action.effective_date || null,
    title: action.title,
    description: action.summary || action.mechanism_label || action.authority_source,
  }));

  return [...visibleLegacyActions, ...normalizedGoverningActions].sort((left, right) =>
...
function formatActionCategorySummary(categories = []) {
...
export default async function PromiseDetailPage({ params, searchParams }) {
  const { slug } = await params;
  ...
  const governingActions = Array.isArray(promise.governing_actions) ? promise.governing_actions : [];
  const visibleLegacyActions = buildVisibleLegacyPromiseActions(
    promise.actions || [],
    governingActions
  );
  const actionModelRecords = buildCombinedActionRecords(promise.actions || [], governingActions);
  const timelineItems = actionModelRecords.map((item) => ({
    action_date: item.action_date,
    title: item.title,
    description: item.description,
  }));
  const evidence = flattenPromiseSources({
    ...promise,
    actions: visibleLegacyActions,
    governing_actions: governingActions,
  });
  ...
  const actionCount = actionModelRecords.length;
  const rolloutParity = promise.current_admin_rollout_parity || null;
  const operatorPreviewEnabled =
    resolvedSearchParams.admin === "1" ||
    resolvedSearchParams.operator === "1" ||
    Boolean(requestHeaders.get("authorization"));
  const showOperatorRolloutWarning =
    operatorPreviewEnabled && Boolean(rolloutParity?.has_mismatch);
  ...
  const promiseVisualEvidenceByPlacement = (promise.visualEvidence || []).reduce((map, item) => {
  ...
  const getPlacedPromiseVisualEvidence = (placement) =>
    promiseVisualEvidenceByPlacement.get(placement) || [];
  const getPromiseVisualBinding = (placement) =>
    typeof promise.visualEvidenceBindings?.[placement] === "string"
      ? promise.visualEvidenceBindings[placement]
      : "";
    ...
    <main className="space-y-4">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard
          ...
          description={`${actionCount} documented action record${actionCount === 1 ? "" : "s"} currently attached, including ${governingActions.length} governing action${governingActions.length === 1 ? "" : "s"}.`}
          tone="info"
        />
        <MetricCard
      ...
      {showOperatorRolloutWarning ? (
        <Panel prominence="primary" className="overflow-hidden border-amber-500/30 bg-[rgba(60,38,7,0.28)]">
          <SectionHeader
            eyebrow="Operator note"
            title="Legacy score and governing-action rollout are not in parity yet"
            description="Public readers still see the legacy promise-level score while the governing-action path remains unscored or incomplete."
          />
          <div className="grid gap-4 p-4 lg:grid-cols-2">
            <Panel padding="md" className="space-y-3">
              <StatusPill tone="warning">Legacy promise-level score</StatusPill>
              <p className="text-sm leading-7 text-[var(--ink-soft)]">
                {rolloutParity?.legacy_promise_level_score?.state === "scored"
                  ? `This promise still carries a legacy score${rolloutParity?.legacy_promise_level_score?.direction ? ` (${rolloutParity.legacy_promise_level_score.direction.toLowerCase()})` : ""}${Number.isFinite(Number(rolloutParity?.legacy_promise_level_score?.total_score)) ? ` with net score ${formatSignedScore(rolloutParity.legacy_promise_level_score.total_score)}` : ""}.`
                  : "This promise does not currently carry a legacy promise-level score."}
              </p>
              <p className="text-xs leading-6 text-[var(--ink-muted)]">
                Legacy outcome count: {rolloutParity?.legacy_promise_level_score?.outcome_count || 0}
              </p>
            </Panel>
            <Panel padding="md" className="space-y-3">
              <StatusPill tone="warning">Linked governing-action state</StatusPill>
              <p className="text-sm leading-7 text-[var(--ink-soft)]">
                Current governing-action score state:{" "}
                <span className="font-semibold text-white">
                  {humanizeToken(rolloutParity?.linked_governing_action_score_state?.state)}
                </span>
                .
              </p>
              {rolloutParity?.source_evidence_gap_reason ? (
                <p className="text-sm leading-7 text-[var(--ink-soft)]">
                  Gap: {rolloutParity.source_evidence_gap_reason}
                </p>
              ) : null}
              <p className="text-xs leading-6 text-[var(--ink-muted)]">
                Recommended next action: {humanizeToken(rolloutParity?.recommended_next_action)}
              </p>
            </Panel>
          </div>
        </Panel>
      ...
      <Panel prominence="primary" className="overflow-hidden">
        <SectionHeader
          ...
          title={promise.tracked_but_not_scored ? "Tracked but not scored yet" : "Linked governing actions"}
          description={
            promise.tracked_but_not_scored
              ? "This promise is still being tracked as an attributable commitment, but EquityStack has not yet published a governing action with scoreable implementation evidence."
              : "Promises and governing actions stay separate here. The records below show the actual mechanisms EquityStack has linked to this promise."
          }
        />
        <div className="space-y-4 p-4">
          {promise.tracked_but_not_scored ? (
            <Panel padding="md" className="space-y-3">
              <StatusPill tone="warning">Tracked but not scored</StatusPill>
              <p className="text-sm leading-7 text-[var(--ink-soft)]">
                Promise language alone does not enter the impact-scoring pipeline. EquityStack waits for a documented mechanism such as an executive order, agency action, rule, funding move, staffing change, enforcement shift, or court action with measurable downstream implementation.
              </p>
            </Panel>
          ) : null}

          {governingActions.length ? (
            <div className="grid gap-4 lg:grid-cols-2">
              {governingActions.map((action) => (
                <Panel key={action.id} padding="md" className="space-y-3">
                  <div className="flex flex-wrap gap-2">
                    <StatusPill tone="info">{humanizeToken(action.action_type)}</StatusPill>
                    <StatusPill tone="info">{humanizeToken(action.status)}</StatusPill>
                    <StatusPill tone="info">{formatSourceQualityLabel(action.best_source_quality)}</StatusPill>
                    <StatusPill tone="info">{formatVisibilityStateLabel(action.visibility_state)}</StatusPill>
                    <StatusPill tone="info">{formatScoreEligibilityLabel(action.score_eligibility)}</StatusPill>
                  </div>
                  <h3 className="text-base font-semibold text-white">{action.title}</h3>
                  <p className="text-sm leading-6 text-[var(--ink-soft)]">
                    {action.summary || action.mechanism_label || "Linked governing action"}
                  </p>
                  <p className="text-sm leading-6 text-[var(--ink-soft)]">
                    Actor: {action.actor_name || "Unknown"} • Authority: {action.authority_source || "Not yet specified"}
                  </p>
                  {summarizeEvidenceRoles(action.sources || action.action_sources).length ? (
                    <p className="text-sm leading-6 text-[var(--ink-soft)]">
                      Evidence roles: {summarizeEvidenceRoles(action.sources || action.action_sources).join(" • ")}
                    </p>
                  ) : null}
                  {formatActionCategorySummary(action.categories).length ? (
                    <p className="text-sm leading-6 text-[var(--ink-soft)]">
                      Impact domains: {formatActionCategorySummary(action.categories).join(" • ")}
                    </p>
                  ) : null}
                  {action.parent_action_title ? (
                    <p className="text-sm leading-6 text-[var(--ink-soft)]">
                      {humanizeToken(action.chain_relationship_type)}: {action.parent_action_title}
                    </p>
                  ) : null}
                  <p className="text-xs leading-6 text-[var(--ink-muted)]">
                    {describeScoreEligibility(action.score_eligibility)}
                  </p>
                  {action.score_eligibility !== "scored" ? (
                    <div className="space-y-1">
                      <p className="text-xs leading-6 text-[var(--ink-soft)]">
                        {CURRENT_ADMIN_PUBLIC_SCORE_WAITING_MESSAGE}
                      </p>
                      {action.next_check_hint ? (
                        <p className="text-xs leading-6 text-[var(--ink-muted)]">
                          {action.next_check_hint}
                        </p>
                      ) : null}
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <MethodologyCallout
              title="Why a public action may still be unscored"
              description={CURRENT_ADMIN_METHOD_STATEMENT}
              href="/research/how-black-impact-score-works"
              linkLabel="Open scoring methodology"
            />
            <DiscoveryGuidancePanel
              eyebrow="Score labels"
              title="Eligible does not mean scored"
              description="These labels describe evidence stage, not whether the page is complete enough to read."
      ...
      <PromisePanel id="status" className={`${localSectionOffsetClass} space-y-5`}>
        ...
        <div className="rounded-lg border border-[var(--line)] bg-[rgba(11,20,33,0.92)] p-4">
          ...
          {getPlacedPromiseVisualEvidence("status").length ? (
            <div className="mt-6 space-y-4">
              {getPromiseVisualBinding("status") ? (
                <p className="text-sm leading-7 text-[var(--ink-soft)]">
                  {getPromiseVisualBinding("status")}
                </p>
              ) : null}
              <VisualEvidenceBlock items={getPlacedPromiseVisualEvidence("status")} />
            </div>
          ) : null}
        ...
        <SourceTrustPanel
          ...
          completenessLabel={`${actionModelRecords.length} actions / ${(promise.outcomes || []).length} outcomes`}
