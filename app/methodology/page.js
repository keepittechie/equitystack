...
const SITE_SURFACES = [
  ...
  },
  {
    title: "Promises",
    description:
      "Promise pages track public commitments and visible follow-through. They are accountability records, not substitutes for downstream impact analysis, and promise-only records can remain public without being scored.",
  },
  {
    title: "Policies",
    description:
      "Policy pages are the main record layer for laws, court actions, executive actions, and outcomes that can be tied to Black impact analysis. During the current-administration rollout, governing actions fill that same mechanism role for live presidency records.",
  },
  {
...
const FRAMING_RULES = [
  ...
  },
  {
    ...
    description:
      "Promises are framed as commitments that can be compared with action and outcomes. Delivery status and Black impact are kept related but distinct, and a promise alone never becomes scoreable just because it was stated.",
  },
  {
    title: "Related laws and executive actions",
    description:
      "Legislation, executive orders, agency actions, judicial blocks, and other governing mechanisms are connected only when public evidence supports the relationship. Weak or missing lineage should be read as incomplete, not silently resolved.",
  },
  {
...
const SCORE_DIMENSIONS = [
  ...
  {
    title: "Public ranking score",
    description:
      "President pages now expose three related numbers: the raw average score, the coverage-adjusted score, and the overall ranking score. The public leaderboard orders presidents by the overall ranking score so sparse records do not outrank fuller ones just because their averages are cleaner.",
  },
  {
  ...
  },
  {
    ...
    description:
      "Promise status is a public-accountability layer, not a substitute for downstream impact. Delivered, partial, failed, or blocked promise status stays distinct from the Black Impact Score, and `eligible for scoring` still does not mean `already scored`.",
  },
  {
...
const CURRENT_ADMIN_READING_RULES = [
  {
    label: "Promise layer",
    tone: "default",
    title: "Intent stays separate from mechanism",
    description:
      "A sourced promise can remain public as an accountability record even before EquityStack can tie it to a real governing action.",
  },
  {
    label: "Governing action",
    tone: "info",
    title: "Real government actions are modeled separately",
    description:
      "Executive orders, agency actions, rules, budget or staffing actions, enforcement moves, and judicial actions are tracked as governing actions so the site does not treat slogans like policies.",
  },
  {
    label: "Score gate",
    tone: "warning",
    title: "Public does not always mean scoreable",
    description:
      "A governing action can be visible before it is scoreable. EquityStack waits for implementation and measurable population-impact evidence before moving from hold for evidence to scoring review.",
  },
];
      ...
      <section className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
        <div className="rounded-lg border border-[var(--line)] bg-[rgba(11,20,33,0.92)] p-4">
          <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--accent)]">
            Current-administration method
          </p>
          <h2 className="mt-4 text-2xl font-semibold text-white">
            Public promise tracking and governing-action tracking are intentionally separate
          </h2>
          <p className="mt-3 text-sm leading-7 text-[var(--ink-soft)]">
            {CURRENT_ADMIN_METHOD_STATEMENT}
          </p>
          <p className="mt-3 text-sm leading-7 text-[var(--ink-soft)]">
            During the current-administration rollout, promise pages remain public accountability records, governing
            actions carry the real mechanism layer, and legacy current-admin outcomes can still remain visible while
            the new governing-action score path is reconciled. That coexistence is deliberate and should be read as an
            auditability feature, not as hidden incompleteness.
          </p>
        </div>
        <DiscoveryGuidancePanel
          eyebrow="Current-admin reading rules"
          title="How to read live current-administration records"
          description="These labels exist so readers can tell the difference between intent, real governing action, evidence readiness, and finished scoring."
          items={CURRENT_ADMIN_READING_RULES}
        />
      </section>
      ...
      <section className="public-two-col-rail grid gap-6 xl:grid-cols-[1fr_1fr]">
        <div className="rounded-lg border border-[var(--line)] bg-[rgba(11,20,33,0.92)] p-4">
          ...
          <div className="mt-5 grid gap-3 text-sm leading-7 text-[var(--ink-soft)]">
            ...
            <div className="rounded-lg border border-[var(--line)] bg-[rgba(18,31,49,0.52)] p-4">
              Public ranking uses an overall ranking score, not the raw average alone. Raw average shows the presidency&apos;s direct scored record before coverage adjustment, while the overall ranking score is the number used to order presidents on public pages.
            </div>
          </div>
