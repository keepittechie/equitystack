...
export default async function PresidentProfilePage({ params }) {
  ...
  const researchCoverage = buildResearchCoverage({
    ...
    hasScore: Number.isFinite(
      Number(president.score ?? president.normalized_score_total ?? president.direct_normalized_score)
    ),
    ...
    <main className="space-y-4">
      ...
      <Panel prominence="primary" className="overflow-hidden">
        <div className="grid gap-0 xl:grid-cols-[minmax(0,1fr)_22rem]">
          ...
          <aside className="grid content-start gap-3 p-4">
            {imageSrc ? (
              <div className="relative mx-auto h-48 w-36 overflow-hidden rounded-lg border border-[var(--line)] bg-[rgba(18,31,49,0.5)] sm:h-52 sm:w-40 md:h-56 md:w-44 xl:mx-0 xl:h-auto xl:w-full xl:max-w-[180px] xl:aspect-[3/4]">
                <Image src={imageSrc} alt={presidentName} fill className="object-cover object-top" />
              </div>
            ) : null}
            <MetricCard
              label="Black Impact Score"
              value={formatScore(president.score ?? president.normalized_score_total ?? president.direct_normalized_score)}
              description="Public overall ranking score for this presidency."
              prominence="primary"
              tone="info"
              showDot
            />
      ...
      <Panel id="overview" prominence="primary" className={`${localSectionOffsetClass} overflow-hidden`}>
        ...
        <div className="space-y-4 p-4">
          ...
          <WhyThisRankPanel
            president={president}
            eyebrow="Why this rank?"
            title="How this profile is ordered on the public leaderboard"
          />
