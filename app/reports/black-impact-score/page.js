...
import WhyThisRankPanel from "@/app/components/public/WhyThisRankPanel";
import {
  MetricCard,
  Panel,
  SectionHeader,
...
function PresidentCredibilityPanel({
  ...
  const rankExplanation = buildPresidentRankExplanation({
    ...president,
    direction_breakdown: president.counts_by_direction,
  });
                  ...
                  >
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      ...
                      <div className="flex flex-wrap justify-end gap-2">
                        ...
                        <MetaPill>Overall {formatNormalizedScore(president.normalized_score)}</MetaPill>
                        {president.display_score != null ? (
                          <MetaPill>Coverage-adjusted {formatNormalizedScore(president.display_score)}</MetaPill>
                        ) : null}
                      </div>
                    </div>
                    <p className="mt-3 text-sm leading-7 text-[var(--ink-soft)]">
                      {rankExplanation.outcome_count} outcomes • {rankExplanation.confidence_level} confidence •{" "}
                      {rankExplanation.coverage_penalty_applied
                        ? "extra low-coverage penalty applied"
                        : "no extra low-coverage penalty"}
                    </p>
                    <p className="mt-1 text-sm leading-7 text-[var(--ink-soft)]">
                      Raw average {rankExplanation.raw_average_score.toFixed(2)} → overall ranking{" "}
                      {rankExplanation.overall_ranking_score.toFixed(2)}.
                    </p>
                  </div>
                );
              })()
        ...
        <div className="card-muted rounded-[1.25rem] p-4">
          ...
          {strongestPositive ? (
            <div className="mt-3">
              ...
              <p className="text-sm text-[var(--ink-soft)] mt-2">
                Overall ranking score: {formatNormalizedScore(strongestPositive.normalized_score)}
              </p>
            </div>
          ) : (
        ...
        <div className="card-muted rounded-[1.25rem] p-4">
          ...
          {strongestNegative ? (
            <div className="mt-3">
              ...
              <p className="text-sm text-[var(--ink-soft)] mt-2">
                Overall ranking score: {formatNormalizedScore(strongestNegative.normalized_score)}
              </p>
            </div>
          ) : (
            <p className="text-sm text-[var(--ink-soft)] mt-3">No comparison data is available.</p>
          ...
          <section key={president.president_slug} className="card-muted rounded-[1.25rem] p-4">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {president.raw_normalized_score != null ? (
                  <MetaPill>Raw average {formatNormalizedScore(president.raw_normalized_score)}</MetaPill>
                ) : null}
...
function normalizeOutcomePresident(president) {
  ...
  return {
    ...
    normalized_score: headlineScore,
    overall_ranking_score: president.overall_ranking_score ?? president.ranking_score_total ?? null,
    raw_normalized_score: president.raw_average_score ?? president.normalized_score_total,
    ranking_score_total: president.ranking_score_total ?? president.overall_ranking_score ?? null,
    ...
    direct_score_confidence:
      president.direct_score_confidence ?? president.evidence_confidence ?? president.score_confidence ?? null,
    ...
    display_score:
      president.coverage_adjusted_score ?? president.display_score_total ?? president.display_score ?? null,
    coverage_adjusted_score:
      president.coverage_adjusted_score ?? president.display_score_total ?? president.display_score ?? null,
    score_confidence: president.evidence_confidence ?? president.score_confidence ?? null,
    evidence_confidence: president.evidence_confidence ?? president.score_confidence ?? null,
    ...
    <>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <Panel padding="md" className="space-y-3">
          <StatusPill tone="info">Current-administration score gate</StatusPill>
          <p className="text-sm leading-7 text-[var(--ink-soft)]">
            {CURRENT_ADMIN_METHOD_STATEMENT}
          </p>
          <p className="text-sm leading-7 text-[var(--ink-soft)]">
            Eligible governing actions can appear in score-ready reporting before they are actually scored. Legacy
            current-admin outcomes remain visible during rollout, so some public promise pages still show legacy score
            context alongside newer governing-action evidence states.
          </p>
        </Panel>
        <DiscoveryGuidancePanel
          eyebrow="How to read this report"
          title="Scoring labels are evidence states, not shortcuts"
          description="Use these cues when the report links back to current-administration records that are public but still withheld from the scored governing-action layer."
            ...
            <section key={president.president_slug} className="card-surface p-4">
              ...
              <div className="grid gap-4 md:grid-cols-4 mt-6">
                <ScoreCard
                  ...
                  subtitle={
                    president.display_score != null
                      ? `Primary public ranking score; coverage-adjusted score ${formatNormalizedScore(president.display_score)}`
                      : "Primary public ranking view across presidents"
                  }
                />
                <ScoreCard
                  label="Raw Score"
