...
import WhyThisRankPanel from "./WhyThisRankPanel";
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-b border-white/6 last:border-b-0">
                ...
                <td className="px-5 py-4 text-[var(--ink-soft)]">{item.year_enacted || NOT_AVAILABLE}</td>
                <td className="px-5 py-4 text-[var(--ink-soft)]">{item.president || item.primary_party || NOT_AVAILABLE}</td>
                <td className="px-5 py-4 text-[var(--ink-soft)]">{item.impact_direction || NOT_AVAILABLE}</td>
                <td className="px-5 py-4 text-white">{item.impact_score ?? NOT_AVAILABLE}</td>
        ...
        return (
          ...
          >
            ...
            >
              ...
              <ScoreBadge value={item.impact_score ?? NOT_AVAILABLE} label="Impact Score" />
            </div>
            {item.summary ? (
              <p
                className={`${
  ...
  return (
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      {items.map((item) => {
        ...
        const rankExplanation = buildPresidentRankExplanation(item);
        ...
        return (
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <ScoreBadge
                value={item.score ?? item.normalized_score_total ?? item.direct_normalized_score ?? NOT_AVAILABLE}
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-wrap items-start gap-3 xl:justify-end">
                <ScoreBadge
                  value={item.score ?? item.direct_normalized_score ?? NOT_AVAILABLE}
                  label="Overall ranking score"
                />
              </div>
                ...
                <div className="rounded-lg border border-[var(--line)] bg-[rgba(18,31,49,0.52)] px-4 py-3">
                  ...
                  <p className="mt-1 text-sm leading-7 text-[var(--ink-soft)]">
                    {rankExplanation.coverage_penalty_applied
                      ...
                      : "No extra low-coverage penalty applied."}
                  </p>
                </div>
              </div>
            </Link>
              ...
              return (
                <tr key={`${item.id || item.slug}-${index}`} className="border-b border-white/6 last:border-b-0">
                  ...
                  <td className="px-5 py-4 text-[var(--ink-soft)]">
                    {item.topic || item.category || NOT_AVAILABLE}
                  </td>
                  <td className="px-5 py-4 text-[var(--ink-soft)]">
                    {item.status || item.impact_direction || NOT_AVAILABLE}
                  </td>
                  <td className="px-5 py-4 text-white">
                    {item.score ?? item.impact_score ?? NOT_AVAILABLE}
                  </td>
                </tr>
              );
            })}
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-b border-white/6 last:border-b-0">
                ...
                <td className="px-5 py-4 text-[var(--ink-soft)]">
                  {item.publisher || NOT_AVAILABLE}
                </td>
                <td className="px-5 py-4 text-[var(--ink-soft)]">
                  {item.source_type || NOT_AVAILABLE}
                </td>
                <td className="px-5 py-4 text-[var(--ink-soft)]">
                  <div>
                ...
                <td className="px-5 py-4 text-[var(--ink-soft)]">
                  {item.trust_label || item.confidence_label || NOT_AVAILABLE}
                </td>
                <td className="px-5 py-4 text-[var(--ink-soft)]">
                  {formatRenderableDate(item.published_date) || NOT_AVAILABLE}
                </td>
              </tr>
            ))}
          </tbody>
            ...
            {items.map((item, index) => {
              ...
              const directionLabel =
                typeof formatDirection === "function"
                  ...
                  : item.impact_direction || item.status || NOT_AVAILABLE;
              ...
              return (
                <tr key={`${item.slug || item.id}-${index}`} className="border-b border-white/6 last:border-b-0">
                  ...
                  <td className="px-5 py-4 text-[var(--ink-soft)]">
                    {formatRenderableDate(item.date) || formatRenderableDate(item.latest_action_date) || NOT_AVAILABLE}
                  </td>
