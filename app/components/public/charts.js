...
function ChartSurface({ children }) {
  ...
  return (
    <div className="mt-auto h-[280px] min-h-[280px] w-full min-w-0 md:h-[320px] md:min-h-[320px] xl:h-[340px] xl:min-h-[340px]">
      {hasMounted ? (
        children
      ) : (
        <div
          aria-hidden="true"
          className="h-full w-full rounded-[1rem] border border-white/6 bg-[rgba(18,31,49,0.35)]"
        />
      )}
    </div>
  );
...
export function ReportChartBlock({ title, description, children }) {
  return (
    <section className="flex h-full min-w-0 flex-col rounded-lg border border-[var(--line)] bg-[rgba(11,20,33,0.92)] p-4 md:p-6">
