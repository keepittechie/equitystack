...
export function PresidentScoreMethodologyNote({
  ...
  const items = [
    {
      label: "Ranking score vs. raw average",
      description:
        "The public leaderboard orders presidents by the overall ranking score, not the raw average alone. Raw average shows the presidency's direct scored record before coverage adjustment and any extra low-coverage penalty.",
    },
    {
    ...
    },
    {
      ...
      description:
        "Public president rankings apply extra damping to LOW and VERY LOW confidence records so a thin set of favorable outcomes does not outrank a fuller presidency with stronger evidence coverage.",
    },
    {
  ...
  return (
    <Panel padding="md" className="h-full">
      ...
      <p className="mt-2 text-sm leading-6 text-[var(--ink-soft)]">
        ...
        The outcome-based layer still carries the most weight. Public ranking
        starts with the raw average, applies coverage adjustment, and then
        applies an extra leaderboard penalty only to sparse LOW and VERY LOW
        confidence presidencies.
      </p>
