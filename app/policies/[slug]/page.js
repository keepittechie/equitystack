...
function formatPolicyScoreOutOfFive(rawScore) {
  ...
  return normalized == null ? "N/A" : `${normalized.toFixed(1)} / 5`;
}
...
export default async function PolicyDetailPage({ params }) {
  ...
  const policyVisualEvidenceByPlacement = (policy.visualEvidence || []).reduce((map, item) => {
  ...
  const getPlacedPolicyVisualEvidence = (placement) =>
    policyVisualEvidenceByPlacement.get(placement) || [];
  const getPolicyVisualBinding = (placement) =>
    typeof policy.visualEvidenceBindings?.[placement] === "string"
      ? policy.visualEvidenceBindings[placement]
      : "";
      ...
      <Panel id="summary" className={`${localSectionOffsetClass} overflow-hidden`}>
        ...
        <div className="space-y-4 p-4">
          ...
          <Panel padding="md" prominence="primary" className="space-y-4">
            ...
            {getPlacedPolicyVisualEvidence("impact-summary").length ? (
              <div className="space-y-4">
                {getPolicyVisualBinding("impact-summary") ? (
                  <p className="text-sm leading-7 text-[var(--ink-soft)]">
                    {getPolicyVisualBinding("impact-summary")}
                  </p>
                ) : null}
                <VisualEvidenceBlock
                  items={getPlacedPolicyVisualEvidence("impact-summary")}
                />
              </div>
            ) : null}
          </Panel>
