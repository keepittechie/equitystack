...
export default async function NarrativesPage() {
  const [compareData, topPoliciesResponse] = await Promise.all([
    getJson("/api/reports/compare"),
    getJson("/api/reports/top-policies"),
  ]);

  const details = Array.isArray(compareData?.details) ? compareData.details : [];
  const topPolicies = Array.isArray(topPoliciesResponse?.records)
    ? topPoliciesResponse.records
    : Array.isArray(topPoliciesResponse)
      ? topPoliciesResponse
      : [];
