...
Public president ranking should be read through three distinct score labels:

- `raw_average_score`
  - the presidency's average scored outcome profile before public ranking penalties
- `coverage_adjusted_score`
  - the raw average after evidence-coverage damping
- `overall_ranking_score`
  - the public ordering score used on presidential ranking surfaces

That final ranking score exists so LOW and VERY LOW confidence presidencies with thin records do not outrank fuller presidencies solely because their raw averages are cleaner.
